...
Fig.\,\ref{fig:3pasSch} shows the PA process outlined previously and adds schematic wavefunctions which show the interparticle spacing which contributes to the overlap integral is situated around the classical turning point of the molecule, known as the Condon point, $R_c$ \cite{Bohn1999,bav00}.
\begin{figure}
	\centerline{
	\includegraphics[width=\textwidth]{PASwithWF.pdf}}
	\caption{Schematic of the photoassociation process}{The photoassociation process relies on favorable overlap between the initial state free particle state and the final bound state. An example incoming wavefunction at collision energy $\epsilon > 0$ is shown coupled to a vibrationally excited bound-state wavefunction in the upper potential. The inset shows the probability density of the excited states and the Condon point (radius) for each bound state.}
	\label{fig:3pasSch}
\end{figure}
Contribution to the overlap in the region $r < R_c$ is negligible due to the fast oscillations in the interior region of both wavefunctions.
Furthermore, the Condon point moves inward as the binding energy of the bound state molecules is increased. 
This causes overlap with the initial scattering state to decrease and consequently a decrease in the rate of molecular formation may be observed \cite{Krems2009a,Julienne2009a,nse06,Jones2006}.
...
% Summary
Although the general idea is straightforward, a rigorous theoretical understanding of the process requires discussion of several key topics related to the behavior of ultracold gases.
...
\begin{equation}\label{eq:number}
   N(t)= \frac{N_0 \rm{e}^{-\Gamma t}}
   		{\displaystyle 1 + \frac{2 N_0 \langle K \rangle V_2}{\Gamma V_1^2}(1-\rm{e}^{-\Gamma t}) }
\end{equation}
...
This is typically a good approximation for photoassociation with alkali atoms using dipole-allowed transitions as the consideration of single-particle kinetic energy may be neglected due to the natural width of the transition dominating any small shifts due to Doppler effect from center-of-mass motion or photon recoil.
On the other hand, photoassociation near narrow intercombination line transitions may be sensitive to Doppler shifts and photon recoil due to the long-lifetime of the excited state that have a naturally narrow transition linewidth.
This narrow linewidth may then be on the order of the Doppler and photon recoil shifts, which will effect the observed photoassociative lineshape \cite{Ciuryo2004, Borkowski2014a, Nicholson2015a, Reschovsky2018, Pachomov2017}.

PAS near narrow narrow intercombination transitions is described by considering both the center-of-mass momentum of particle pairs in addition to the relative momentum.
Once more, consider Eq.\,\ref{eq:kLoss} written as a thermal average,
\begin{equation} \label{eq:onePasMom}
	 \langle K \rangle_\text{thermal} = \int_0^{\infty} d^3\vec{p}_1 \int_0^{\infty} d^3\vec{p}_2 \,\frac{2 \pi \hbar^2}{|\vec{p}_1 - \vec{p}_2| \mu} \, f_\text{p,two}( \vec{p}_1, \vec{p}_2 ) \, \vert  S(\vec{p}_1, \vec{p}_2, \omega_1, \omega_2, \cdots) \vert^2  
\end{equation}
...
Eq.\,\ref{eq:onePasMom} may be reduced to the earlier equations for $\langle K \rangle_\text{thermal}$ by performing a coordinate transform into center-of-mass and relative coordinates and integrating over the center-of-mass\footnote{This is shown in App.\,\ref{sec:standardDist}}.
