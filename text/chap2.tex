...
While optical dipole traps are useful for efficient evaporation and thermalization of an ultracold gas, experiments with Feshbach molecules in ODTs suffer from a high rate of inelastic collisions. \cite{Kohler2006}. 
The association of these molecules in an optical lattice has been shown to slow these processes since atoms must tunnel from site to site and can be pinned in sufficiently deep potentials \cite{Thalhammer2006,Syassen2007}.
In order to gain a quantitative understanding of these processes we will briefly outline the relevant lattice physics for ultracold bosons in an optical lattice.
...
With this configuration we estimate a maximum depth of  >$30$E$_r$ is achievable in an isotropic lattice. 
	\begin{figure} 
		\centerline{
		\includegraphics[width=\textwidth]{lattice_depth.png}}
		\caption{Lattice depth calibration}{Calibration was performed using the 2-band Kapitza-Dirac technique. Maximum photodiode voltage for each arm is 10\,V}
		\label{fig:latDepth}
	\end{figure}
...
The retro-reflection is optiized via the kapitza-dirac method.
For shallow lattice depths, the amplitude of the population in the $\pm$1 plane waves is linear vs lattice depth.
This requires an iterative appraoch to ensure that you have 

As our lattice is free space, the first order alignment of the retroreflection is to look over a long distance and overlap the incoming and return beam s best as possible.
This tens to be close enough that you are able to observe some diffraction effects when performing Kapitza-Dirac s 
...
As was discusses in Sec.\;\ref{sec:lattice}, the free space eigenstates are not the eigenstates of the lattice Hamiltonian. 
...
Instead, we exponentially ramp on the lattice over 100\,ms which reduces heating caused by the ramp. As shown in Fig.\;\ref{fig:heatingRates}, we observe a large condensate fraction after similarly ramping the lattice back down. 
...
From Fig.\;\ref{fig:heatingRates}, we see that there is not an appreciable loss of atoms over a one second timescale, supporting our estimate that off-resonant scattering is unimportant on these timescales.
	\begin{figure}
	\centerline{
...
Huge amount of electronic needed to run this experiment. 
...
Software is a custom built Labview appication which implements a synchronous state machine.
...
This is tough to acheive and led to the Rydbberg apparatus using separate coils for the red and blue MOTs.
...
Give data showing that the pulseblasters are synced (if I have it)
 ...
 This circuit will trigger the experiment to run with a similar phase relationship to the 60 Hz line from shot to shot in an attempt to common-mode reject noise that may originate from stray electrical noise. The additional logic gates ensure that the pulseblaster trigger at the same time.

\subsubsection{Pneumatic actuated mirror mounts}

One challenge of using a 532\,nm opticl lattice is that this wavelength is right in the middle of the 461\,nm and 689\,nm MOT beams and must operate at high power.
This places prohibitive constraints of the availability of passive optical components which might combine these three wavelengths.
Tnstead we use a couple of pneumatic actuators to pull out the senesitive optics before using the 532\,nm lattice.
We only have to do this in the vertical direction becuae we can sneak by along the horizontal paths but vertically the optical access is restricted so we must pull back the specialty waveplate as well as both MOT mirrors on either side of the chamber. 

As expected, this does impart phonons to the table but in practice we find that the system is fairly robust against these small "kicks".
However, occasiaonlly the air pressure has to adjusted if the lasers are misbehaving.

The electronic and air system used for driving this syste is given in \hl{some fig}.
