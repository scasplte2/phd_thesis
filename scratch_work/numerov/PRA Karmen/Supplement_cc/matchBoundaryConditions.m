...
for(i1=1:nopen)
%	Fn(open(i1),open(i1))=sqrt(pi*Rgrid(npoints)/(2*k(open(i1))))*besselj(ls(open(i1))+1/2,k(open(i1))*Rgrid(npoints));
%	Fn1(open(i1),open(i1))=sqrt(pi*Rgrid(npoints-1)/(2*k(open(i1))))*besselj(ls(open(i1))+1/2,k(open(i1))*Rgrid(npoints-1));
%	Gn(open(i1),open(i1))=sqrt(pi*Rgrid(npoints)/(2*k(open(i1))))*bessely(ls(open(i1))+1/2,k(open(i1))*Rgrid(npoints));
%	Gn1(open(i1),open(i1))=sqrt(pi*Rgrid(npoints-1)/(2*k(open(i1))))*bessely(ls(open(i1))+1/2,k(open(i1))*Rgrid(npoints-1));
...
for(i1=1:nclosed)
%	Gn(closed(i1),closed(i1))=sqrt(pi*Rgrid(npoints)/(2*k(closed(i1))))*besselk(ls(closed(i1))+1/2,k(closed(i1))*Rgrid(npoints),1);
%	Gn1(closed(i1),closed(i1))=sqrt(pi*Rgrid(npoints-1)/(2*k(closed(i1))))*besselk(ls(closed(i1))+1/2,k(closed(i1))*Rgrid(npoints-1),1)*exp(k(closed(i1))*(Rgrid(npoints)-Rgrid(npoints-1)));
...
%use column scaling, which improves beyond gerrits believe
B=Gn1-Qn*Gn;
nrm=zeros(nstates,1);
for(i1=1:nstates)
	nrm(i1)=norm(B(:,i1));
