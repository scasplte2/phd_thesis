...
Description:
SCILAB routines for the developed potential following algorithms.
...
Filenames: README.TXT + the 14 files:

	prop.sci	Contains the propagation algorithms.
	seig.sci	Diagonalization wrapper.
	exp_airy.sci	Contains helper functions for the asymptotic expansion of the Airy functions.
	match.sci	Contains functions for matching to scattering (S-matrix) boundary conditions.
	dvr.sci		Contains sinc-DVR function for the Secrest and Johnson test problem.
	link.sci	Executes required files in scilab
	job1.sh		test jobs with output
	job1.out	--
        job2.sh         --
        job2.out        --
        job3.sh         --
...
The propagators are contained in the file prop.sci

the function numerov is the renormalized Numerov algorithm.
Usage is: Q=numerov(Rgrid,E,mu,wmat_fun,wmat_vars)
	Rgrid is a vector of radial grid points. This should be equidistant.
...
Included test jobs are named job1.sh through job3.sh
...
or the variables Ra, Rb and npoint
