...
% Types of potentials
V_modLJ = @(C6, C8, C10, C12, r) C12./r.^12 - C6./r.^6 - C8./r.^8 - C10./r.^10; % modified Lennard - Jones potential
V_LJ    = @(A, C6, C12, r, el) A*(C12./r.^12 - C6./r.^6) + el*(el + 1)./r.^2;
V_morse = @(De, beta, re, r) De*(exp(-2*beta*(r-re)) - 2*exp(-beta*(r-re)));% Morse potential
...
kLoc = @(r) k(m, Einc - V(r)).^2; % local wavevector (i.e. propagator coefficient)
...
R(1) = 1e-10; %arbitrary
R(2) = (1 + k(m, V(z(1)) - Einc)*d)*R(1); % assume linear first step
...
for i = 3:length(z)
    R(i)      = numerov(d, kLoc(z([i-2 i-1 i])), R([i-2 i-1]));
    %Psi(i)    = numerov(d, kLoc(z([i-2 i-1 i])), Psi([i-2 i-1]).*z([i-2 i-1]));
end

% Find the value at the asymptotic solution to get normalization
x = kLoc(z(end)).*z([end end-1]);
...
A = u(z(end), kLoc(z(end)), el, coeffs)/R(end);
