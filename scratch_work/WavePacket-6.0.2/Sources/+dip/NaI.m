...
log.disp('  ')
log.disp('****************************************************************')
log.disp('Dipole moments of the lowest two diabatic states of')
log.disp('NaI. The explicit formulas are given in            ')
log.disp('J.Chem.Soc.,Faraday Trans. 93:977.                 ')
log.disp('                                                   ')
log.disp('Values for the overlap integrals are tabulated in  ')
log.disp('J.Chem.Phys. 17:1248                               ')
log.disp('****************************************************************')
...
% dip.spherical_2D, i.e. fill only hamilt.d_x.grid for the
...
if space.n_dim > 2
    log.error('NaI dipole moment can only handle 1-2 dimensions')
end

if hamilt.coupling.n_eqs ~= 2
    log.error('NaI dipole moment is only for 2 coupled states')
end
...
if space.n_dim == 2
	if ~isfield(hamilt.dip, 'R_dof') || isempty(hamilt.dip.R_dof)
		log.error('NaI dipole moment requires degree of freedom to be set.')
	end
	...
	angleval = space.dvr{L_dof};
end

Rval = space.dvr{hamilt.dip.R_dof};

[p, t] = convert(Rval, mu_Na, mu_I);

hamilt.d_x.grid{1,1} = Rval;
hamilt.d_x.grid{2,2} = Rval/2 .* mod_overlap(p,t) ./ (1 + overlap(p,t).^2);
hamilt.d_x.grid{1,2} = Rval .* (overlap(p,t) + mod_overlap(p,t))...
            ./ sqrt(2 * (1+overlap(p,t).^2));

if space.n_dim == 2
    hamilt.d_x.grid{1,1} = hamilt.d_x.grid{1,1} .* angleval;
    hamilt.d_x.grid{2,2} = hamilt.d_x.grid{2,2} .* angleval;
    hamilt.d_x.grid{1,2} = hamilt.d_x.grid{1,2} .* angleval;
end
