...
classdef interp < dip.generic & handle
    ...
    methods (Access = public)
       ...
       % Constructor: Set default values
        ...
        function init_dip (obj)
            ...
            if space.n_dim>1
                if length(obj.n_pts)~=space.n_dim
                    log.error ('Inconsistent number of dimensions of tabulated data')
                end
            end
            ...
            switch obj.pol
                ...
                case 3
                    p = 'z';
                otherwise
                    log.error ('Polarization direction has to be 1,2,3 for x,y,z')
            end
            ...
            fid = fopen(obj.file);
            if fid == -1
                    log.error (['Input data file missing : ' obj.file])
            else
                fclose(fid);
            end
        ...
        function disp(obj)
            global hamilt space
            log.disp ('Interpolating tabulated values of dipole moment')
            log.disp ('***************************************************************')
            log.disp ( ' ' )
            log.disp (['Conversion factor for coordinates : ' num2str(obj.pos_conv)])
            log.disp (['Conversion factor for dipoles     : ' num2str(obj.dip_conv)])
            log.disp (['Interpolation method              : '         obj.method])
            if space.n_dim>1
                log.disp (['Number of data points in each dimension  : ' int2str(obj.n_pts)])
            end
            if obj.row==obj.col
                log.disp (['Read potential (' hamilt.coupling.labels{obj.row} ') from data file : ' obj.file])
            else
                log.disp (['Read potential (' hamilt.coupling.labels{obj.row},', ',hamilt.coupling.labels{obj.col} ') from data file : ' obj.file])
            end
        end
