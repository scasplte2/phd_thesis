...
classdef H2O < dip.generic & handle
    ...
    methods (Access = public)
        ...
        function init_dip (obj)
            ...
            if space.n_dim ~= 2
                log.error ('This dipole function is only for 2 dimensions')
            end
            
            if hamilt.coupling.n_eqs ~= 2
                log.error ('This dipole function is only for 2 states')
            end
        ...
        function disp(obj)
            log.disp ('Model function for transition dipole moment of H2O')
            log.disp ('***************************************************************')
            log.disp (' ')
            log.disp (['Range parameter          : ' num2str(obj.b)])
            log.disp (['Equilibrium distance H-O : ' num2str(obj.r0)])
        end
