...
if s.type == '.'
    switch s.subs
        ...
        case 'kin_max'
            retval = 1 / ( 2*obj.mass_R*min(space.dof{obj.dof_R}.x_grid)^2 );

            if isempty(obj.r_0)
                retval = retval + 1 / ( 2*obj.mass_r*min(space.dof{obj.dof_r}.x_grid)^2 );
            else
        ...
        case 'grid_exp'
            retval = obj.grid_exp;
        otherwise
            log.error ( ['Tried to get invalid field ' s.subs 'in kin.jacobi object'] );
    end
else
    log.error( 'Tried unsupported access form of class kin.jacobi' );
end
