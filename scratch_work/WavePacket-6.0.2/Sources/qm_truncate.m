...
log.disp (' ')
log.disp ('-------------------------------------------------------------')
log.disp (' Truncating A, B, N, C matrices; also x_i, x_e state vectors ')
log.disp (' http://sourceforge.net/p/wavepacket/matlab/wiki/Reference.Programs.qm_truncate');
log.disp ('-------------------------------------------------------------')
log.disp (' ')
log.disp(['truncating to ' int2str(dim) ' modes:'])
...
switch lower(method)
    case 's' % singular perturbation for A, N, C
        ...
        log.disp('singular perturbation approximation')
...
log.disp(['Saving truncated matrices A, B, N, C and densities to file : ' filename '.mat'])
