       ...
       % Constructor: Set default values
        function obj = C9A (choice)
            switch choice
                ...
                case 'S1-Sx'
                otherwise
                    log.error ('Wrong specification of electronic states')
            end
            obj.state = choice;
        end
        ...
        function init_pot (~)
            ...
            if space.n_dim ~= 1
                log.error ('This potential is for one degrees of freedom only')
            end
        ...
        function disp(obj)
            log.disp ('Potential for 9-(N-carbazolyl) anthracene (C9A)')
            log.disp ('***************************************************************')
            log.disp (' ')
            log.disp (['Electronic state(s) : ' obj.state])
