       ...
       % Constructor: Set default values
        ...
        function init_pot (obj)
            ...
            if length (obj.d_e) ~= space.n_dim
                log.error ('Wrong length of Morse parameters')
            end
            if length (obj.r_e) ~= space.n_dim
                log.error ('Wrong length of Morse parameters')
            end
            if length (obj.alf) ~= space.n_dim
                log.error ('Wrong length of Morse prameters')
            end
            if (any (obj.d_e)<=0) || (any (obj.r_e)<=0) || (any (obj.alf)<=0) 
                log.error ('Morse parameters should be positive')
            end
        end
        ...
        function disp(obj)
            global space
            log.disp ('Morse oscillator')
            log.disp ('***************************************************************')
            for k=1:space.n_dim
                log.disp (' ')
                if space.n_dim > 1
                    log.disp ( [ 'Dimension : ' num2str(k) ])
                end
                log.disp ( [ 'Dissociation energy    : ' num2str(obj.d_e(k)) ] )
                log.disp ( [ 'Equilibrium position   : ' num2str(obj.r_e(k)) ] )
                log.disp ( [ 'Range parameter (alfa) : ' num2str(obj.alf(k)) ] )
            end
            log.disp ( [ 'Energetic shift        : ' num2str(obj.t_e) ] )
        end
