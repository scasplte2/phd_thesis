...
classdef pendulum < pot.generic & handle
    ...
    methods (Access = public)
        ...
        function disp(obj)
            log.disp ('Quasi-exactly solvable generalized pendulum')
            log.disp ('***************************************************************')
            log.disp ('                                                  ')
            log.disp (' V = xi*csc^2+chi*cot*csc-eta*cos-zeta*cos^2+v_0  ')
            log.disp (' see work by B. Schmidt and B. Friedrich (2014-17)')
            log.disp ('                                                  ')
            log.disp ('Important note on usage for planar pendulum       ')
            log.disp ('    0 <= theta <= 2*pi (or also 4*pi !)           ')
            log.disp ('Important note on usage for spherical pendulum    ')
            log.disp ('    0 <= theta <= pi  and xi = m^2 - 1/4          ')
            log.disp ('    where m is the azimuthal quantum number       ')
            log.disp (' ')
            log.disp ( [ 'csc^2 term  (xi)   : ' num2str(obj.xi) ] )
            log.disp ( [ 'cot*csc term (chi) : ' num2str(obj.chi) ] )
            log.disp ( [ 'Orientation (eta)  : ' num2str(obj.eta) ] )
            log.disp ( [ 'Alignment (zeta)   : ' num2str(obj.zeta) ] )
            log.disp ( [ 'Energy shift (v_0) : ' num2str(obj.v_0) ] )
        end
