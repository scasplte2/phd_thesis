...
classdef interp < pot.generic & handle
    ...
    methods (Access = public)
       ...
       % Constructor: Set default values
        ...
        function init_pot (obj)
            ...
            if space.n_dim>1
                if isempty(obj.n_pts)
                    log.error ('Number of tabulated data has to be specified')
                end
        ...
        function disp(obj)
            global space
            log.disp ('Interpolating tabulated values of potential energy')
            log.disp ('***************************************************************')
            log.disp ( ' ' )
            log.disp (['Conversion factor for coordinates   : ' num2str(obj.pos_conv)])
            log.disp (['Conversion factor for energies      : ' num2str(obj.pot_conv)])
            log.disp (['Interpolation method                : '         obj.method])
            log.disp (['Read tabulated potential from file  : '         obj.file])
            if space.n_dim>1
                log.disp (['Number of points in each dimension  : ' int2str(obj.n_pts)])
            end
