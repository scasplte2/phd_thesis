...
log.disp (' ')
log.disp ('********************************************************************')
log.disp ('Potential energy: NOCl                                 ')
log.disp ('Surface data taken from J. Chem. Phys 97:3199          ')
log.disp ('********************************************************************')


%% Check validity
if hamilt.coupling.n_eqs ~= 2
    log.error ('This potential only for two Schr�dinger equations')
end

if space.n_dim ~= 2 && space.n_dim ~= 3
    log.error ('This potential is only for two or three degrees of freedom')
end
...
if space.n_dim == 2
    ...
    Rgrid = space.dvr{hamilt.pot.R_dof};
    rgrid = ones(size(space.dvr{1})) * jacobi_re(2);
    cgrid = space.dvr{hamilt.pot.c_dof};
else
    Rgrid = space.dvr{hamilt.pot.R_dof};
    rgrid = space.dvr{hamilt.pot.r_dof};
    cgrid = space.dvr{hamilt.pot.c_dof};
...
hamilt.pot.grid{1,1} = k23 * (bl_r{2}-bl_re(2)) .* (bl_r{3}-bl_re(3));
for d = 1:3
    hamilt.pot.grid{1,1} = hamilt.pot.grid{1,1} ...
                              + 0.5*k(d)*(bl_r{d}-bl_re(d)).^2;
end


% Excited state potential
hamilt.pot.grid{2,2} = zeros(size(space.dvr{1}));

for l1 = 0:3
    for l2 = 0:4
        for l3 = 0:6
            hamilt.pot.grid{2,2} = hamilt.pot.grid{2,2} ...
                    + c_param(5*l1 + l2 + 1, l3 + 1) * qv.^l1 .* qd.^l2 .* qt.^l3;
        end
    end
end

hamilt.pot.grid{2,2} = hamilt.pot.grid{2,2} .* (1 - qd);

for l1 = 2:4
    hamilt.pot.grid{2,2} = hamilt.pot.grid{2,2} + a_param(l1-1) .* qv.^l1;
end
