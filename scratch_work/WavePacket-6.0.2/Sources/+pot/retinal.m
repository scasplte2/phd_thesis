...
classdef retinal < pot.generic & handle
    ...
    methods (Access = public)
        ...
        function init_pot (~)
            ...
            if space.n_dim > 2
                log.error ('This potential is only for 1 or 2 dimensions')
            end
        ...
        function disp(obj)
            global space
            log.disp ('Retinal isomerization')
            log.disp ('S.Hahn, G.Stock, JPC B 104(6),1149 (2000)')
            log.disp ('***************************************************************')
            log.disp ( ' ' )
            if obj.row==obj.col
                log.disp ( [ 'Vertical shift      : ' num2str(obj.shift)   ] )
                log.disp ( [ 'Barrier height      : ' num2str(obj.barrier) ] )
                log.disp ( [ 'Frequency           : ' num2str(obj.omega)   ] )
                log.disp ( [ 'Linear term         : ' num2str(obj.kappa)   ] )
            else
                log.disp ( [ 'Coupling constant   : ' num2str(obj.lambda)  ] )
            end
            if space.n_dim == 1
                log.disp ( [ 'Coupling coordinate : ' num2str(obj.xc)      ] )
            end
        end
