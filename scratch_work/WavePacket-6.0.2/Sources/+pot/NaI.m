...
log.disp('  ')
log.disp('****************************************************************')
log.disp('Potential energy surfaces of the lowest two states ')
log.disp('of NaI. The explicit formulas are given in         ')
...
if hamilt.coupling.n_eqs ~= 2
    log.error('NaI potentials are only for two coupled states.')
end

if space.n_dim > 2
    log.error('NaI potentials are only for up to two degrees of freedom.')
end
