    ...
    methods (Access = public)
       ...
       % Constructor: Set default values
        ...
        function init_pot (obj)
            global space
            if space.n_dim ~= 2
                log.error( 'This surface is for 2 dimensions.')
            end
            if length(obj.d_e) ~= 3
                log.error( 'Wrong number of LEPS parameters')
            end
            if length(obj.r_e) ~= 3
                log.error( 'Wrong number of LEPS parameters')
            end
        ...
        function disp(obj)
            log.disp( 'Fixed angle atom/diatom A+BC reactive scattering  ')
            log.disp( '***************************************************************')
            log.disp( '                                                  ')
            log.disp( ['Bending angle       : ' num2str(obj.ang)] )
            for n = 1:3
                log.disp( ' ' )
                log.disp( ['Bond length #       : ' int2str(        n )] )
                log.disp( ['dissociation energy : ' num2str(obj.d_e(n))] )
                log.disp( ['equilibrium distance: ' num2str(obj.r_e(n))] )
                log.disp( ['range parameter     : ' num2str(obj.alf(n))] )
                log.disp( ['Sato parameter      : ' num2str(obj.s_p(n))] )
            end
        end
