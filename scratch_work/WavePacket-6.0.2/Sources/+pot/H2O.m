%*******************************************************************
% Semi-empirical calculation for the PES of water.    
% The bound state is modeled after some Morse         
% oscillators, the excited state based on ab-initio   
% calculations. Fixed bending angle of 104.52 degrees.
% See J. Chem. Phys. 97:8285,( J. Chem. Phys. 88:129 )
%                                                     
% Ground and excited state are joined such that       
% dR(R1 = R_OH, R2 -> infinity) = 0                   
%*******************************************************************
...
classdef H2O < pot.generic & handle
    
    properties (Access = public)
        
        gamma       % constant HOH bending angle
    ...
    methods (Access = public)
        ...
        function obj = H2O
            obj.gamma = 104.52; % constant HOH bending angle
        end
        ...
        function init_pot (~)
            ...
            if hamilt.coupling.n_eqs ~= 2
                log.error ('This potential only for two Schrödinger equations')
            end
            
            if space.n_dim ~= 2
                log.error ('This potential is for 2 degrees of freedom only')
            end
        ...
        function disp(obj)
            ...
            if obj.row==1 && obj.col==1
                ...
                log.disp ('Semi-empirical calculation for the PES of water.  ')
                log.disp ('***************************************************************')
                log.disp ('                                                  ')
                log.disp (['Constant HOH bending angle : ' num2str(obj.gamma)])
        ...
        function V = V(obj,r)
            ...
            R1 = r{1};                   % O-H distance
            R3 = r{2};                   % other O-H distance
            R2 = sqrt(R1.^2 + R3.^2 - 2*cosd(obj.gamma) * R1.* R3);   % H-H distance
            ...
            if obj.row==2 && obj.col==2
                ...
                % Convert the distances to % Angstroem, and the final energy from eV to atomic units.
                R1 = R1 * 0.5292;
                R2 = R2 * 0.5292;
                R3 = R3 * 0.5292;
                ...
                V = omega_fit .* pot_fit + omega_asy .* pot_asy;
                V = V / 27.212; % conversion eV -> Hartree
