...
classdef henon < pot.generic & handle
    ...
    methods (Access = public)
        ...
        function init_pot (~)
            ...
            % Check validity
            if space.n_dim ~= 2
                log.error ('This potential only for two dimensions')
            end
        ...
        function disp(obj)
            log.disp ('Potential energy: Henon-Heiles system in 2 dimensions  ')
            log.disp ('********************************************************************')
            log.disp ('                                                       ')
            log.disp ( [ 'Force constant A       : ' num2str(obj.A) ] )
            log.disp ( [ 'Cubic parameter lambda : ' num2str(obj.L) ] )
            log.disp ( [ 'Saddle height          : ' num2str(obj.A^3/(6*obj.L^2)) ] )
            log.disp ( [ 'Radius of triangle     : ' num2str(obj.A  /   obj.L   ) ] )
        end
