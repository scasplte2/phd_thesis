    ...
    methods (Access = public)
        ...
        function init_pot (obj)
            ...
            if hamilt.coupling.n_eqs ~= 1
                log.error ('This potential only for single Schrödinger equation')
            end
            
            if space.n_dim ~= 3
                log.error ('This potential is for 3 degrees of freedom only')
            end
            
            if ~isa (space.dof{obj.c_dof}, 'grid.legendre')
                log.error ('The potential requires Jacobi coordinates.')
            end
        end
        ...
        function disp(obj)
            log.disp ('***************************************************************')
            log.disp ('High quality MRCI calculation for potential energy')
            log.disp ('surface of  H3+                                   ')
            log.disp ('See J. Chem. Phys. 84:891                         ')
            log.disp ('                                                  ')
            log.disp ('***************************************************************')
            log.disp (' ')
            log.disp ( [ 'angular coordinate : ' num2str(obj.c_dof) ] )
        end
