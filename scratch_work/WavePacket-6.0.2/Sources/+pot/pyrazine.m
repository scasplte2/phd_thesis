...
classdef pyrazine < pot.generic & handle
    ...
    methods (Access = public)
        ...
        function init_pot (~)
            ...
            if space.n_dim ~= 3
                log.error ('This potential is only for 3 dimension')
            end
            
            if hamilt.coupling.n_eqs ~= 2
                log.error ('This potential is only for 2 equations')
            end
        ...
        function disp(obj)
            ...
            if obj.row==1 && obj.col==1
                log.disp ('Ultrafast S2-S1 internal conversion of pyrazine  ')
                log.disp ('*************************************************')
                log.disp ('                                                 ')
                log.disp ( [ 'Force constants omega   : ' num2str(obj.omega) ] )
                log.disp ( [ 'Linear parameters kappa : ' num2str(obj.kappa) ] )
                log.disp ( [ 'Vertical excitation E   : ' num2str(obj.energ) ] )
                log.disp ( [ 'Linear coupling lambda  : ' num2str(obj.lambda) ] )
            else
