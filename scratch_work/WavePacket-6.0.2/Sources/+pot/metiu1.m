...
classdef metiu1 < pot.generic & handle
    ...
    methods (Access = public)
        ...
        function init_pot (~)
            global hamilt space
            if space.n_dim > 1
                log.err('This potential works only for one DOF')
            end
            
            if ~isa(space.dof{1}, 'grid.legendre')
                log.err('This potential works only for Legendre DVR')
            end
        ...
        function disp(obj)
            ...
            log.disp('***************************************************************')
            log.disp('                                                  ')
            log.disp([ 'Parameter A0 : ' num2str(obj.a0) ])
            log.disp([ 'Parameter A1 : ' num2str(obj.a1) ])
            log.disp([ 'Parameter A2 : ' num2str(obj.a2) ])
        end
