       ...
       % Constructor: Set default values
        ...
        function init_pot (obj)
            ...
            if hamilt.coupling.n_eqs ~= obj.N
                log.error ('Number of electronic sites inconsistent')
            end
            ...
            if length (obj.S) ~= space.n_dim
                log.error ('Number of selected phonons inconsistent')
            end
            if any ( obj.S < 1 )
                log.error ('Phonon selection index too small')
            end
            if any ( obj.S > obj.N )
                log.error ('Phonon selection index too large')
            end
...
%             end
        ...
        function disp(obj)
            if obj.row==1 && obj.col==1
                log.disp ('Coupled electron-phonon dynamics: Jahn-Teller and beyond')
                log.disp ('***************************************************************')
                log.disp (' ')
                log.disp ('**************')
                log.disp ('Electrons only')
                log.disp ('**************')
                log.disp (' ')
                log.disp (['Number of electronic sites    : ' int2str(obj.N)])
                log.disp (['Electronic site energy        : ' num2str(obj.U)])
                log.disp (['Electronic coupling strength  : ' num2str(obj.W)])
                log.disp (' ')
                for d=1:obj.N
                    log.disp (['Exciton wavenumber index      : ' int2str(obj.j(d))])
                    log.disp (['Exciton wavenumber (for a=1)  : ' num2str(obj.k(d))])
                    log.disp (['Exciton energy                : ' num2str(obj.e(d))])
                    log.disp (' ')
                end
                log.disp ('************')
                log.disp ('Phonons only')
                log.disp ('************')
                log.disp (' ')
                log.disp (['1-particle harmonic frequency : ' num2str(obj.O)])
                log.disp (' ')
                for d=1:obj.N
                    log.disp (['Phonon wavenumber index       : ' int2str(obj.j(d))])
                    log.disp (['Phonon wavenumber (for a=1)   : ' num2str(obj.k(d))])
                    log.disp (['Phonon harmonic frequency     : ' num2str(obj.o(d))])
                    log.disp (['Frequency ratio               : ' num2str(obj.r(d))])
                    log.disp (' ')
                end
                log.disp ('Selection of phonons')
                log.disp (' ')
                for d=1:length(obj.S)
                    log.disp ([int2str(obj.S(d)) ': ' obj.l{obj.S(d)}])
                end
                log.disp (' ')
                log.disp ('************************')
                log.disp ('Electron-phonon coupling')
                log.disp ('************************')
                log.disp (' ')
                log.disp (['Coupling constant ( JT) : ' num2str(obj.K)])
                log.disp (['Coupling constant (PJT) : ' num2str(obj.L)])
                log.disp (' ')
                for d = 1:length(obj.S)
