...
classdef con_int < pot.generic & handle
    ...
    methods (Access = public)
        ...
        function init_pot (~)
            ...
            if space.n_dim > 2
                log.error ('This potential is only for 1 or 2 dimension')
            end
            
            if hamilt.coupling.n_eqs > 2
                log.error ('This potential is only for 1 or 2 (coupled) channels')
            end
        end
        ...
        function disp(obj)
            ...
            if obj.row==1 && obj.col==1
                ...
                log.disp ('Generic E x e conical intersection example :     ')
                log.disp ('***************************************************************')
                log.disp ('                                                 ')
                log.disp ( [ 'Force constant            omega : ' num2str(obj.omega) ] )
                log.disp ( [ 'Linear JT-coupling        kappa : ' num2str(obj.kappa) ] )
                log.disp ( [ 'Quadr. JT-coupling        gamma : ' num2str(obj.gamma) ] )
                if space.n_dim == 1
                    log.disp ( [ 'Energy gap                delta : ' num2str(obj.delta) ] )
                end
                if obj.x_ci ~= 0
                    log.disp ( [ 'x-shift of conical intersection : ' num2str(obj.x_ci) ] )
                end
                if obj.y_ci ~= 0
                    log.disp ( [ 'y-shift of conical intersection : ' num2str(obj.y_ci) ] )
                end
