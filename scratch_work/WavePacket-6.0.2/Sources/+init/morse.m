    ...
    methods (Access = public)
        ...
        function init_dof (obj)
           ...
           if ( obj.d_e<=0 || obj.r_e<=0 || obj.alf<=0 || obj.m_r<=0 )
                log.error ('Morse prameters should be positive')
           end
           ...
           if (obj.n_q>obj.n_b)
               log.error ('quantum number exceeds number of bound states')
           end
        ...
        function disp(obj)
            ...
            log.disp ('Morse oscillator eigenstate')
            log.disp ('***************************************************************')
            log.disp (' ')
            log.disp (['Dissociation energy     : ' num2str(obj.d_e)])
            log.disp (['Equilibrium position    : ' num2str(obj.r_e)])
            log.disp (['Range parameter (alfa)  : ' num2str(obj.alf)])
            log.disp (['(Reduced) mass          : ' num2str(obj.m_r)])
            log.disp (' ')
            log.disp (['Number of bound states  : ' int2str(obj.n_b+1)])
            log.disp (['Harmonic frequency      : ' num2str(obj.omg)])
            log.disp (['Anharmonicity           : ' num2str(obj.chi)])
            log.disp (' ')
            log.disp (['Quantum number          : ' int2str(obj.n_q)])
            log.disp (['Energy                  : ' num2str(obj.e_n)])
