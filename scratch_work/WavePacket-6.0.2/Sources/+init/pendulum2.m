...
classdef pendulum2 < init.generic & handle
    ...
    methods (Access = public)
        ...
        function init_dof (obj)
            ...
            if ~isa(space.dof{obj.dof},'grid.fft')
                log.error ('This wavefunction allowed only for FFT grids!')
            end
            ...
            if round(obj.kappa)~=obj.kappa
                    log.error ('kappa should be integer')
            end
            
            if (obj.kappa<1)
                log.error ('kappa should be positive')
            end
            ...
            switch obj.irrep
                case 'A1'
                    if mod(dx,2*pi)
                        log.error ('A1 irrep available only for 2pi interval')
                    end
                    if ~mod(obj.kappa,2)
                        log.error ('A1 irrep is only for odd kappa')
                    end
                    T = A1(obj); % Setup matrix T
                    
                case 'A2'
                    if mod(dx,2*pi)
                        log.error ('A2 irrep available only for 2pi interval')
                    end
                    if ~mod(obj.kappa,2)
                        log.error ('A2 irrep is only for odd kappa')
                    end
                    if obj.kappa < 3
                        log.error ('A2 irrep not for kappa=1')
                    end
                    T = A2(obj); % Setup matrix T
                    
                case 'B1'
                    if mod(dx,4*pi)
                        log.error ('B1 irrep available only for 4pi interval')
                    end
                    if mod(obj.kappa,2)
                        log.error ('B1 irrep is only for even kappa')
                    end
                    T = B1(obj); % Setup matrix T
                    
                case 'B2'
                    if mod(dx,4*pi)
                        log.error ('B2 irrep available only for 4pi interval')
                    end
                    if mod(obj.kappa,2)
                        log.error ('B2 irrep is only for even kappa')
                    end
            ...
            if obj.n_q > length (T)-1
                log.error ('The quantum number is too large for this kappa!')
            end
        ...
        function disp(obj)
            ...
            log.disp ('Analytic eigenfunctions of a generalized planar pendulum                                ')
            log.disp ('***************************************************************')
            log.disp (' ' )
            log.disp ( ['Potential parameter beta   : ' num2str(obj.beta)] )
            log.disp ( ['Potential parameter kappa  : ' int2str(obj.kappa)] )
            log.disp (' ' )
            log.disp ( ['Potential parameter eta    : ' num2str(obj.beta*obj.kappa)] )
            log.disp ( ['Potential parameter zeta   : ' num2str(obj.beta^2)] )
            log.disp (' ' )
            log.disp ( ['Irreducible representation : '         obj.irrep] )
            log.disp (' ' )
            log.disp ( ['Quantum number             : ' int2str(obj.n_q)] )
            log.disp ( ['Eigenenergy                : ' num2str(-obj.D(obj.n_q+1))])
