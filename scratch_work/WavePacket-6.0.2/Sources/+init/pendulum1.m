...
classdef pendulum1 < init.generic & handle
    ...
    methods (Access = public)
            ...
            switch obj.multiple
                case 1 % Only even integer orders r
                    if obj.order~=2*round(obj.order/2)
                        log.error ('Order of Mathieu functions not compatible with m!')
                    end
                ...
                otherwise
                    log.error ('Fractional order Mathieu functions not yet implemented!')
            end
        ...
        function disp(obj)
            ...
            log.disp ('Pendular state wavefunction')
            log.disp ('***************************************************************')
            log.disp ('                                                  ')
            log.disp ('Ordinary (angular) Mathieu functions              ')
            log.disp ('                                                  ')
            log.disp ('ce_r (eta,q) with r>=0 : cosine elliptic functions')
            log.disp ('se_r (eta,q) with r>=1 :   sine elliptic functions')
            log.disp ('                                                  ')
            log.disp ('solving a Schrödinger equation for potential      ')
            log.disp ('                                                  ')
            log.disp ('       V(R) = V0/2 [ 1 + cos (m*(R-R0) ]          ')
            log.disp ('                                                  ')
            log.disp ('for periodic coordinate 0 < R <= 2*pi where the   ')
            log.disp ('multiplicity m indicates number of maxima/minima  ')
            log.disp ('and where eta=m*(R-R0)/2 and q=2V0/m^2            ')
            log.disp ('                                                  ')
            log.disp ('m=1: Only pi-periodic solutions: Only even r      ')
            log.disp ('m=2: All 2pi-periodic solutions: Integer r        ')
            log.disp ('m>2: Requires also fractional orders of r         ')
            log.disp ('     Yields complex solutions: not yet implemented')
            log.disp ('  ')
            
            switch lower(obj.parity)
                case ('c')
                    log.disp ( 'Cosine elliptic' )
                 case {'s'}
                    log.disp ( 'Sine elliptic' )
                 case {'l'}
                    log.disp ( 'Localized combination of cosine and sine elliptic' )
                otherwise
                    log.error ('Wrong choice for parity of Mathieu function')
            end
            log.disp ( [ 'Mathieu order r      : ' int2str( obj.order)    ] )
            log.disp ( [ 'Multiplicity m       : ' int2str( obj.multiple) ] )
            log.disp ( [ 'Potential barrier V0 : ' num2str( obj.barrier)  ] )
            log.disp ( [ 'Angular shift R0     : ' num2str( obj.shift)    ] )
