...
classdef interp < handle
    ...
    methods (Access = public)
        ...
        function disp(obj)
            global space
            log.disp ('Interpolation of initial wave function from tabulated data')
            log.disp ('***********************************************************************')
            log.disp (' ')
        ...
        function wave_corr (obj, psi)
            ...
            for m=1:hamilt.coupling.n_eqs
                if hamilt.coupling.n_eqs>1
                    log.disp (['State: ' hamilt.coupling.labels{m}])
                end
                ...
                if fid == -1
                    log.disp (['Input data file not available : ' obj.file_re{m}])
                    log.disp ('Assuming real part of wave function to be zero');
                    log.disp (' ')
                else
                    fclose(fid);
                ...
                if fid == -1
                    log.disp (['Input data file not available : ' obj.file_im{m}])
                    log.disp ('Assuming imaginary part of wave function to be zero');
                    log.disp (' ')
                else
                    fclose(fid);
                    psi.dvr{m} = psi.dvr{m} + ...
