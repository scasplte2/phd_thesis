    ...
    methods (Access = public)
        ...
        function init_dof (obj)
            if ~isempty(obj.state) && ~isempty(obj.coeffs)
                log.error ('Specify either "state" or "coeffs" but not both!')
            end
           if isempty(obj.state) && isempty(obj.coeffs)
                log.error ('Either "state" or "coeffs" have to be specified!')
            end
        end
        ...
        function disp(obj)
            ...
            log.disp ('FBR ("momentum" grid eigenfunction.               ')
            log.disp ('***************************************************************')
            log.disp (' ' )
            if ~isempty(obj.state) 
                log.disp ( ['Selecting the ' int2str(obj.state) '-th eigenstate'])

            end
            if ~isempty(obj.coeffs)
                log.disp( ['Using coefficient vector : ' num2str(obj.coeffs) ])
            end
