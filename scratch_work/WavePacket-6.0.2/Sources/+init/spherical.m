...
classdef spherical < init.generic & handle
    ...
    methods (Access = public)
        ...
        function init_dof (obj)
            ...
            if obj.l<0
                log.error ('quantum number l must not be negative')
            end
            if obj.m<0
                log.error ('quantum number m must not be negative')
            end
            if obj.m>obj.l
                log.error ('quantum number m must not exceed l')
            end
        ...
        function disp(obj)
            ...
            log.disp ('Spherical harmonic wavefunction')
            log.disp ('***************************************************************')
            log.disp ('   ' )
            log.disp ( ['Angular momentum quantum number l : ' int2str(obj.l)] )
            log.disp ( ['Azimuthal quantum number        m : ' int2str(obj.m)] )
            log.disp ( ['Divide by sqrt(sin(Theta))        : ' int2str(obj.sqst)] )
