...
log.disp (' ')
log.disp ('------------------------------------------------------------------------------')
log.disp (' Balancing transformation of A, B, N, C matrices; also x_i, x_e state vectors ')
log.disp (' http://sourceforge.net/p/wavepacket/matlab/wiki/Reference.Programs.qm_balance')
log.disp ('------------------------------------------------------------------------------')
log.disp (' ')
...
log.disp(['Method for solving generalized Lyapunov equation: ' reduce.balance.method])

if ~isfield (reduce.balance,'transform')
    reduce.balance.transform = 'srbt';
end
log.disp(['Method of balancing transformation: ' reduce.balance.transform])

if ~isfield(reduce.balance,'BN_scale')
    reduce.balance.BN_scale = 1;
end
log.disp(['Scaling B-vector, N-matrix: ' num2str(reduce.balance.BN_scale)])

% Load full matrices from data file  
% downscale B and N such that generalized Lyapunov equations are solvable
log.disp (['Loading bilinear system matrices from file: ' filename '.mat'])
load(filename)
n = size(bilinear.A,1);
log.disp (['Dimensionality of original system: ' int2str(n)])
...
switch lower(reduce.balance.A_stable)
    case 'ssu' 
        ...
        log.disp ('Method ''ssu'' : splitting stable from unstable part')     
        if ~isfield (reduce.balance,'A_split')
            reduce.balance.A_split = 1;
        end
        n1 = reduce.balance.A_split; 
        log.disp (['Dimension of unstable part : ',int2str(n1)])
        log.disp (' ')
    ...
    case 'evs' 
        log.disp('Method ''evs'': eigenvalue shift of A')
        
        % value of shift
        if ~isfield(reduce.balance,'A_shift')
            reduce.balance.A_shift = 1e-6;
        end
        log.disp (['Value of shift : ' num2str(reduce.balance.A_shift)])
        log.disp (' ')
    ...
    otherwise
        log.error (['Wrong choice of stabilization method : ' reduce.balance.A_stable])
...
log.disp('------------------------------------');
log.disp('Solve generalized Lyapunov equation ');
log.disp('to get controllability Gramian W_C  ');
log.disp('A*WC + WC*Ap + N*WC*Np + B*Bp = 0   ');
switch lower (reduce.balance.method)
    ...
    otherwise
        log.error (['Wrong choice of GLYAP solver method : ' reduce.balance.method])
end

%% Observability Gramian from generalized Lyapunov equation
log.disp('------------------------------------');
log.disp('Solve generalized Lyapunov equation ');
log.disp('to get observability Gramian W_O    ');
log.disp('Ap*WO + WO*A + Np*WO*N + Cp*C = 0   ');
switch lower (reduce.balance.method)
    case 'iter'
...
switch lower (reduce.balance.transform)
    ...
    otherwise
        log.error (['Wrong choice of balancing transform method : ' reduce.balance.transform])
end

% Plot details of balancing transformation
vis.balance (Sigma,WC,WO)
...
math.pseudo(balanced.S,balanced.T)
...
log.disp(['Saving balanced matrices A, B, N, C and vectors x,y to file : ' filename '_b.mat'])
