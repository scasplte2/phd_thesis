...
clear global atomic bilinear balanced correlate control reduce expect hamilt info plots space time
% clear functions;
fclose('all');

% Calling a few constructors
global hamilt
...
if toggle
    format longE
    log.disp ('***************************************************************')
    log.disp ('Using atomic units throughout WavePacket!')
    log.disp ('***************************************************************')
    log.disp ('   ')
    log.disp (atomic.m)
    log.disp (atomic.l)
    ...
    log.disp (atomic.I)
    log.disp ('***************************************************************')
    log.disp ('Most important atomic masses')
    log.disp ('***************************************************************')
    log.disp ('   ')
    log.disp (atomic.mass)
    format short
end
