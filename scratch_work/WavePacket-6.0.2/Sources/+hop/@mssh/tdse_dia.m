...
if isempty(obj.psi_old{1})
    for m=1:hamilt.coupling.n_eqs
        ...
        for n=1:hamilt.coupling.n_eqs
            obj.psi_old{m} = obj.psi_old{m} + 1i * state.ham{m,n} .* obj.psi{n} * time.steps.s_delta;
        end
    end
    ...
    for m=1:hamilt.coupling.n_eqs
        for n=1:hamilt.coupling.n_eqs
            H(:,m,n)        = -1i * time.steps.s_delta * state.ham{m,n};
            H_trans(:,n,m)  = H(:,m,n);
        end
    end
    ...
    if(mod(maxdegree,2)==0)
        log.error ( 'maxdegree has to be odd' )
    end
