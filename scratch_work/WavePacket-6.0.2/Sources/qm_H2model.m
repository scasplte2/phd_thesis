...
log.disp (' ')
log.disp ('--------------------------------------------------------------------------------')
log.disp (' Interpolation based H2 model reduction (c) 2013, Breiten/Benner, MPI Magdeburg ')
log.disp (' Reducing A, B, N, C matrices; also initial/equilibrium state vectors, x_i, x_e ')
log.disp (' http://sourceforge.net/p/wavepacket/matlab/wiki/Reference.Programs.qm_H2model   ')
log.disp ('--------------------------------------------------------------------------------')
log.disp (' ')
log.disp(['reducing to ' int2str(dim) ' modes:'])
log.disp (' ')
...
log.disp(['Convergence tolerance for BIRKA iteration: ' num2str(reduce.H2model.conv_tol)])

if ~isfield (reduce.H2model,'max_iter')
    reduce.H2model.max_iter=100;
end
log.disp(['Maximal number of BIRKA iterations: ' int2str(reduce.H2model.max_iter)])

if ~isfield (reduce.H2model,'method')
    reduce.H2model.method='iter';
end
log.disp(['Method for solving generalized Sylvester equation: ' reduce.H2model.method])

if ~isfield(reduce.H2model,'BN_scale')
    reduce.H2model.BN_scale = 1;
end
log.disp(['Scaling B-vector, N-matrix: ' num2str(reduce.H2model.BN_scale)])
...
switch lower(reduce.H2model.A_stable)
    ...
    case 'evs' 
        ...
        log.disp (['Value of shift : ' num2str(reduce.H2model.A_shift)])
        log.disp (' ')
    ...
    otherwise
        log.error (['Wrong choice of stabilization method : ' reduce.H2model.A_stable])
...
while(err > reduce.H2model.conv_tol && iter < reduce.H2model.max_iter)
    ...
    switch lower (reduce.H2model.method)
        ...
        otherwise
            log.error (['Wrong choice of GSYLV solver method : ' reduce.H2model.method])    
    end
    ...
    log.disp([int2str(iter) '-th BIRKA step: conv. crit. = ' num2str(err)])
...
global bilinear
bilinear = redmat;
log.disp(['Saving reduced matrices A, B, N, C and vectors x,y to file : ' filename '_h' int2str(dim) '.mat'])
