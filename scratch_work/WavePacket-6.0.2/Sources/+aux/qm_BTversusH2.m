...
if switchoff<1 % Not necessary if energy/dipole matrices from external sources
    qm_setup(); state=wave; qm_init(state); qm_bound(state);   qm_cleanup; 
    qm_setup(); state=wave; qm_init(state); qm_matrix(state);  qm_cleanup; 
end
...
if ~isempty (truncate)
    ...
    for len=1:control.y.dim
        plot (time.steps.m_grid,reference(len,:)-reference(len,1),...
    ...
    for k=1:length(truncate)
        ...
        for len=1:control.y.dim
            plot(time.steps.m_grid,real(control.y.forward(len,:)-control.y.forward(len,1)),...
...
if ~isempty (H2reduce)
    ...
    for len=1:control.y.dim
        plot (time.steps.m_grid,real(reference(len,:)-reference(len,1)),...
            'LineStyle', thisplot.patterns{1}, ...
            'LineWidth', thisplot.l_thin,...
            'Color',thisplot.colors(len,:))
    end
