...
log.disp (['Value of A-shift : ' num2str(reduce.H2error.A_shift)])

if ~isfield(reduce.H2error,'BN_scale')
    reduce.H2error.BN_scale = 1;
end
log.disp(['Scaling B-vector, N-matrix: ' num2str(reduce.H2error.BN_scale)])

if ~isfield (reduce.H2error,'method')
    reduce.H2error.method='iter';
end
log.disp(['Method for GLYAP solving: ' reduce.H2error.method])
...
if ~isempty (truncate)
    ...
    for k=1:length(truncate)
        ...
        log.disp('   ')
        log.disp('--------------------------------')
        log.disp('Calculate error norm (BT method)')
        log.disp('--------------------------------')
        log.disp('   ')
...
if ~isempty (singular)
    ...
    for k=1:length(singular)
        ...
        log.disp('   ')
        log.disp('--------------------------------')
        log.disp('Calculate error norm (SP method)')
        log.disp('--------------------------------')
        log.disp('   ')
...
if ~isempty (H2reduce)
    ...
    for k=1:length(H2reduce)
        ...
        log.disp('   ')
        log.disp('--------------------------------')
        log.disp('Calculate error norm (H2 method)')
        log.disp('--------------------------------')
        log.disp('   ')
...
switch reduce.H2error.method
    ...
    otherwise
        log.error (['Wrong choice of GLYAP solver method : ' reduce.H2error.method])
end
...
H2err = sqrt(H2err);
log.disp(['H2 error norm = ', num2str(real(H2err))])
log.disp('   ')
