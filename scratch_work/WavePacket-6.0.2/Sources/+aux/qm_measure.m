...
log.disp (' ')
log.disp ('-------------------------------------------------------------')
log.disp (' Reads the solution x_t from bilinear control equation and   ')
log.disp (' undoes balanced truncation transformation and               ')
log.disp (' reconstructs the density operator (in matrix form) and      ')
log.disp (' calculates various correlation measures                     ')
log.disp ('                                                             ')
log.disp ('-------------------------------------------------------------')
log.disp (' ')
...
if isfield(control.correlation, 'purity')
    correlate.purity = zeros(time.steps.m_number, 1);
    correlate.impure = zeros(time.steps.m_number, 1);
...
if isfield(control.correlation, 'entropy')
    correlate.entropy = zeros(time.steps.m_number, 1);
    correlate.max_ent = zeros(time.steps.m_number, 1);
...
if isfield(control.correlation, 'coherence')
            ...
            for len = 1:length(control.correlation.coherence)
                ...
                correlate.coherence{len} = zeros(time.steps.m_number, 1);
                correlate.max_coher{len} = zeros(time.steps.m_number, 1);
...
if isfield(control.correlation, 'purity')
    plot (time.steps.m_grid, correlate.purity,...
        'LineStyle', '-', ...
        'LineWidth', plots.style.line.thick,...
        'Color',     plots.style.colors(1,:))
    plot (time.steps.m_grid, correlate.impure,...
        'LineStyle', ':', ...
        'LineWidth', plots.style.line.thick,...
        'Color',     plots.style.colors(1,:))
end
...
if isfield(control.correlation, 'entropy')
    plot (time.steps.m_grid, correlate.entropy,...
        'LineStyle', '-', ...
        'LineWidth', plots.style.line.thick,...
        'Color',     plots.style.colors(2,:))
    plot (time.steps.m_grid, correlate.max_ent,...
        'LineStyle', ':', ...
        'LineWidth', plots.style.line.thick,...
        'Color',     plots.style.colors(2,:))
end
...
if isfield(control.correlation, 'coherence')
    for len = 1:length(control.correlation.coherence)
        plot (time.steps.m_grid, correlate.coherence{len},...
            'LineStyle', '-', ...
            'LineWidth', plots.style.line.thick,...
            'Color',     plots.style.colors(2+len,:))
        plot (time.steps.m_grid, correlate.max_coher{len},...
            'LineStyle', ':', ...
            'LineWidth', plots.style.line.thick,...
            'Color',     plots.style.colors(2+len,:))
    end
