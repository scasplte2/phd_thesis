...
%    after.
%
% As input, give the directories and file names (values of wave.save.dir and
% wave.save.file) for both calculations, and the range of time steps that you
...
context1 = wave.load(dir1, file1, true);
context2 = wave.load(dir2, file2);

if nargin < 5 || isempty(steps)
	steps = 1:context1.time.steps.m_number;
end
...
for index = 1:numel(steps)
	context1 = wave.load(context1, steps(index));
	context2 = wave.load(context2, steps(index));

    wf1 = context1.wf.grid;
    wf2 = context2.wf.grid;

	for m = 1:context1.hamilt.coupling.n_eqs
		norm1(index) = norm1(index) + sum(abs(wf1{m}(:)).^2 .* context1.space.weight(:));
        norm2(index) = norm2(index) + sum(abs(wf2{m}(:)).^2 .* context2.space.weight(:));

        [trafo xxx] = wave.transform(wf1{m}, size(wf2{1}), false);
        corr(index)  = corr(index) ...
                + sum(conj(trafo(:)) .* wf2{m}(:) .* context2.space.weight(:));
    end
end
