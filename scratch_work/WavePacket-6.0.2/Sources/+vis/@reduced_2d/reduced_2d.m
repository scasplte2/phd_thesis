    ...
    methods (Access = public)
        ...
        function show_plot (obj, state, step)
            ...
            if ~mod(space.n_dim,2)==0
                log.error ('This visualization for even number of dimensions only')
            end
            ...
            if strcmpi(class(state),'wave')
                ...
                for k = 1:space.n_dim
                    if ~isa(space.dof{k},'grid.fft')
                        log.error ('Reduced densities not (yet!) available for non-FFT grids')
                    end
                end
                ...
                for m = 1:hamilt.coupling.n_eqs
                    ...
                    if expect.pop.cha{m}(step)>expect.min_pop
                        ...
                        switch space.n_dim
                           ...
                           otherwise
                                log.error ('Reduced Q/M densities only up to 4 dimensions')
                        end
            ...
            for k = 1:space.n_dim/2
                ...
                switch(lower(obj.represent))
                    ...
                    otherwise
                        log.error ('Only the DVR representation is available')
                end
