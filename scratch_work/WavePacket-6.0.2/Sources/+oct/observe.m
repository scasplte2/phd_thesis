...
switch lower(action)
    ...
    case 'initial'
        ...
        if ~isfield(bilinear,'C') && ~isfield(bilinear,'D')
            log.error ('either cell vector C or D need to exist')
        end
        
        % However, C and D shouldn't both exist (to be generalized later ...)
        if isfield(bilinear,'C') && isfield(bilinear,'D')
            log.error ('cell vectors C and D should not both exist')
        end
...
%                     log.error ('All observable matrices D should be Hermitian')
%                 end
            end
        end
    ...
    otherwise
        log.error('Calling observe.m with invalid keyword')
end
