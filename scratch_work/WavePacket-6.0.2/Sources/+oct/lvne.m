...
switch lower (control.initial.choice)
    case 'pure' % pure state
        ...
        rho(ii,ii) = 1;
        log.disp(['LvNE initial density: pure state = ', int2str(control.initial.pure)])
    case 'cat' % Schroedinger cat state: coherent superposition
        ...
        rho(ii,jj) = 1/2;
        rho(jj,ii) = 1/2;
        log.disp(['LvNE initial density: cat state = ', int2str(control.initial.cat)])
    case 'mixed' % Incoherent superposition
        ii = control.initial.mixed(1)+1;
        jj = control.initial.mixed(2)+1;
        rho(ii,ii) = 1/2;
        rho(jj,jj) = 1/2;
        log.disp(['LvNE initial density: mixed state = ', int2str(control.initial.mixed)])
    case 'thermal' % thermal (Boltzmann) distribution
        ...
        log.disp(['LvNE initial density: thermal with kBT = ', num2str(control.initial.temperature)])
        for n=1:dim
            log.disp([int2str(n-1) ' : ' num2str(real(boltz(n)))])
        end
    otherwise
        log.error (['Wrong choice of LvNE initial conditions: ' control.initial.choice])
end
log.disp(' ')
...
log.disp(['LVNE equilibrium density: thermal with kBT = ', num2str(control.lvne.temperature)])
for n=1:dim
    log.disp([int2str(n-1) ' : ' num2str(rho(n,n))])
end
log.disp(' ')
