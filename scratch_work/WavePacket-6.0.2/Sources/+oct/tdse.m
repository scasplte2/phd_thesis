...
switch lower (control.initial.choice)
    case 'pure' % pure state
        ...
        psi(ii) = 1;
        log.disp(['TDSE initial state: pure state = ', int2str(control.initial.pure)])
    case 'cat' % Schroedinger cat state
        ii = control.initial.cat(1)+1;
        jj = control.initial.cat(2)+1;
        psi(ii) = 1/sqrt(2);
        psi(jj) = 1/sqrt(2);
        log.disp(['TDSE initial state: cat state = ', int2str(control.initial.cat)])
    otherwise
        log.error (['Wrong choice of TDSE initial conditions: ' control.initial.choice])
end
...
log.disp(['TDSE "equilibrium" : ground state'])
log.disp(' ')
