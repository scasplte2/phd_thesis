...
if isfield(bilinear,'C')
    ...
    if bilinear.Q{control.optimal.terminal}
        ...
        log.disp (['After ' int2str(iter) ' iterations (C2): J = ' num2str(j.target) ' - ' num2str(j.cost) ' = ' num2str(j.total)])
    else
        j.total = 2*j.target-j.cost;
        log.disp (['After ' int2str(iter) ' iterations (C1): J = 2*' num2str(j.target) ' - ' num2str(j.cost) ' = ' num2str(j.total)])
    end
end
...
if isfield(bilinear,'D')
    ...
    j.target = math.real(j.target);
    j.total = j.target-j.cost;
    log.disp (['After ' int2str(iter) ' iterations (D): J = ' num2str(j.target) ' - ' num2str(j.cost) ' = ' num2str(j.total)])
end
