...
data = load (filename);
log.disp(['Loading tabulated data from file : ' filename])

% Get number of data points
n_dat = size(data,1);   % Number of rows
n_dim = size(data,2)-1; % Number of columns, reduced by one
log.disp(['Number of data points found      : ' int2str(n_dat)])
log.disp(['Number of dimensions detected    : ' int2str(n_dim)])
            
% Check for consistency (for n_dim>1 only)
if n_dim>1
    if n_dat ~= prod(n_pts)
        log.error ('Inconsistent number of data points')
    end
end
...
switch n_dim
    case 1
        log.disp(['First data pair      : ' num2str(r{1}(1)) ' / ' num2str(f(1),8)])
        log.disp(['Last data pair       : ' num2str(r{1}(n)) ' / ' num2str(f(n),8)])
        log.disp(['Minimum              : ' num2str(r{1}(s)) ' / ' num2str( c  ,8)])
        log.disp(['Maximum              : ' num2str(r{1}(t)) ' / ' num2str( d  ,8)])
        fi = interp1( r{1}, f, ri{1}, method, 'extrap' );
        
    case 2
        log.disp(['First data triple    : ' num2str(r{1}(1)) ' / ' num2str(r{2}(1)) ' / ' num2str(f(1),8)])
        log.disp(['Last data triple     : ' num2str(r{1}(n)) ' / ' num2str(r{2}(n)) ' / ' num2str(f(n),8)])
        log.disp(['Minimum              : ' num2str(r{1}(s)) ' / ' num2str(r{2}(s)) ' / ' num2str( c  ,8)])
        log.disp(['Maximum              : ' num2str(r{1}(t)) ' / ' num2str(r{2}(t)) ' / ' num2str( d  ,8)])
    ...
    case 3
        log.disp(['First data quadruple : ' num2str(r{1}(1)) ' / ' num2str(r{2}(1)) ' / ' num2str(r{3}(1)) ' / ' num2str(f(1),8)])
        log.disp(['Last data quadruple  : ' num2str(r{1}(n)) ' / ' num2str(r{2}(n)) ' / ' num2str(r{3}(n)) ' / ' num2str(f(n),8)])
        log.disp(['Minimum              : ' num2str(r{1}(s)) ' / ' num2str(r{2}(s)) ' / ' num2str(r{3}(s)) ' / ' num2str( c  ,8)])
        log.disp(['Maximum              : ' num2str(r{1}(t)) ' / ' num2str(r{2}(t)) ' / ' num2str(r{3}(t)) ' / ' num2str( d  ,8)])
        % Swapping the first two arguments, see above
        fi = interp3( r{2}, r{1}, r{3}, f, ri{2}, ri{1}, ri{3}, method );
                
    otherwise
        log.disp(['Minimum              : ' num2str(c,8)])
        log.disp(['Maximum              : ' num2str(d,8)])
