...
n=size(A,1);
log.disp ('Solving generalized Lyapunov equation by iteration method')
log.disp (['for dimensionality = ' int2str(n)])

if ~isfield(params,'dev_max')
    params.dev_max = 1e10;
end
log.disp (['Initial maximal deviation = ' num2str(params.dev_max)])

if ~isfield(params,'conv_tol')
    params.conv_tol = 1e-10;
end
log.disp (['Convergence tolerance = ' num2str(params.conv_tol)])

if ~isfield(params,'max_iter')
    params.max_iter = 10;
end
log.disp (['Max. number of iterations = ' int2str(params.max_iter)])
...
log.disp('Iteration sum|W_i-W_{i-1}| max|W_i-W_{i-1}| sum|W_i|    rank(W_i)')
for iter=1:params.max_iter
    ...
    log.disp(['    ' num2str(iter, '%3.0d') '      ' num2str(dev_sum, '%11.4e')...
        '       ' num2str(dev_max, '%11.4e') '       ' ...
            num2str(sum_mat, '%11.4e') '  ' num2str(rkcrhs)])
    if ((dev_sum < params.conv_tol) && (dev_sum/sum_mat < params.conv_tol))
        log.disp(['Convergence reached after ' num2str(iter) ' iterations'] )
        break
    end
    if (params.dev_max < dev_max)
        log.disp(['Smallest deviation after ' num2str(iter-1) ' iterations'] )
        X=Xold;
        break
    end
...
log.disp(['GLYAP1 terminated after ' int2str(iter) ' iterations with error norm = ' num2str(norm(Deviate))]);
log.disp ('   ')
end
