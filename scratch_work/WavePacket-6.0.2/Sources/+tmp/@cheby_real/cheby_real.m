...
classdef cheby_real < handle
    ...
    methods (Access = public)
        ...
        function disp(obj)
            global time
            log.disp ('Chebychev polynomial expansion in real time')
            log.disp ('***************************************************************')
            log.disp (' ')
        ...
        function wave_init (obj, ~)
            ...
            if time.steps.m_alpha <= 40
                log.error ('Kosloff number should not be below 40, then it becomes inefficient')
            end
