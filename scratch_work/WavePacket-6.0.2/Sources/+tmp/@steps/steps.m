...
classdef steps < handle
    ...
    methods (Access=public)
        ...
        function init (obj)
            ...
            log.disp ('***************************************************************')
            log.disp ('Temporal discretization: Main steps and sub steps')
            log.disp ('***************************************************************')
            log.disp ( ' ' )
            log.disp ( [ 'Index of first step  : ' int2str(obj.m_start)   ] )
            log.disp ( [ 'Index of last step   : ' int2str(obj.m_stop)    ] )
            log.disp ( [ 'Number of steps      : ' int2str(obj.m_number-1)] )
            log.disp ( ' ' )
            log.disp ( [ 'Size of steps        : ' num2str(obj.m_delta)   ] )
            log.disp ( [ 'Total time           : ' num2str(obj.t_total)   ] )
            log.disp ( ' ' )
            log.disp ( [ 'Number of sub-steps  : ' int2str(obj.s_number)  ] )
            log.disp ( [ 'Size of sub-steps    : ' num2str(obj.s_delta)   ] )
            log.disp ( ' ' )
            ...
            if isfield (hamilt,'range')
                if isfield (hamilt.range,'delta')
                    ...
                    log.disp ( [ 'Kosloff alpha (main steps)    : ' num2str(obj.m_alpha)  ] )
                    log.disp ( [ 'Kosloff alpha (sub steps)     : ' num2str(obj.s_alpha)  ] )
                    log.disp ( ' ' )
                end
            end
