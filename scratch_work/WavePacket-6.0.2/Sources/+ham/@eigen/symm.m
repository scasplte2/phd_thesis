...
N = space.n_tot;
if mod(N,2)
    log.error ('Symmetry adaption only for even number of grid points')
end
    ...
    case {'grid.legendre','grid.hermite'}
        ...
        switch lower(obj.symmetry)
            case 'g' % N/2 rows in transformation matrix
                log.disp ('Even (g) parity (A_g irrep of C_i group) - GAU-LEG-HER')
                obj.transform = [main anti];
            case 'u' % N/2 rows in transformation matrix
                log.disp ('Even (g) parity (A_g irrep of C_i group) - GAU-LEG-HER')
                obj.transform = [main -anti];
            otherwise
                log.error ('Symmetry should be "g" (=even) or "u" (=odd) for GAU-LEG-HER')
        end
        
    case 'grid.fft' % Note absence of grid point at space.dof{1}.r_max
        switch obj.symmetry
            case 'g' % N/2+1 rows in transformation matrix
                log.disp ('Even (g) parity (A_g irrep of C_i group) - FFT')
                main = eye ( N/2-1 );
                anti = fliplr(main);
            ...
            case 'u' % N/2-1 rows in transformation matrix
                log.disp ('Odd (u) parity (A_u irrep of C_i group) - FFT')
                main = eye ( N/2-1 );
                anti = fliplr(main);
                fill = zeros(N/2-1,1);
                obj.transform = [fill main fill -anti];
            case 'A1'
                log.disp ('1st (A_1) irrep of C_2v group - FFT')
                main = eye ( N/4-1 );
                anti = fliplr(main);
            ...
            case 'B1'
                log.disp ('2nd irrep of C_2v group - FFT')
                main = eye ( N/4-1 );
                anti = fliplr(main);
                init = zeros(1,N); init(1)=1; init(N/2+1)=-1;
                fill = zeros(N/4-1,1);
            ...
            case 'B2'
                log.disp ('3rd irrep of C_2v group - FFT')
                main = eye ( N/4-1 );
                anti = fliplr(main);
                last = zeros(1,N); last(N/4+1)=1;last(3*N/4+1)=-1;
                fill = zeros(N/4-1,1);
            ...
            case 'A2'
                log.disp ('4th irrep of C_2v group - FFT')
                main = eye ( N/4-1 );
                anti = fliplr(main);
                fill = zeros(N/4-1,1);
                obj.transform = [fill main fill -anti fill main fill -anti];
            otherwise
                log.error ('Symmetry should be "g" (=even) or "u" (=odd) or A1,A2,B1,B2 for FFT')
        end
