...
if ~isempty(obj.R_0)
    ...
    % here.
    obj.kin = space.fbr{obj.dof}.^2 / (2*obj.mass*obj.R_0^2);
else
    % The complicated case: The R value is taken from another DOF whose index
    % resides in obj.R_dof.
    obj.kin = space.fbr{obj.dof}.^2 ./ (2 * obj.mass * space.dvr{obj.R_dof}.^2);
end
...
%% matrix.
obj.kin_expo = exp(-1i * time.steps.s_delta * fraction * obj.kin);
...
if output
    log.disp ('***************************************************************')
    log.disp ( ['Kinetic energy for Gauss-Legendre dof: ' obj.label] )
    log.disp ('***************************************************************')
    log.disp ('                                                  ')
    log.disp ('        1   -2   ^ 2                              ')
    log.disp (' T  =  --- R     L                                ')
    ...
    if isempty(obj.R_dof)
        log.disp ( ['where R is chosen as constant: ' num2str(obj.R_0)] )
    else
        log.disp ( ['where R is taken from the DOF: ' num2str(obj.R_dof)] )
    end
    log.disp ( ['with mass M : ' num2str(obj.mass)] )
    log.disp ('                                                  ')
end
