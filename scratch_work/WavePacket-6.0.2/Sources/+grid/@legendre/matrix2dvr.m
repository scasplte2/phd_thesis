...
% Our task is to change this to DVR.
...
if length(size(fbr)) ~= 2*space.n_dim
    log.error('Input argument is not a matrix')
end
...
if space.n_dim > 1
    permutation = (1:2*space.n_dim);
    permutation(obj.dof+space.n_dim) = 2*space.n_dim;
    permutation(end) = obj.dof + space.n_dim;
