...
if isa(state,'wave') || ( isa(state,'traj') && space.n_dim<3 )
    ...
    % Some logging header
    log.disp ( '***************************************************************' )
    log.disp ( 'Setting up a direct product of the one-dimensional grids       ' )
    log.disp ( '***************************************************************' )
    log.disp ( ' ' )
    log.disp ( [ 'Dimensionality of problem          : ' int2str(space.n_dim) ] )
    log.disp ( [ 'Total number of grid points        : ' int2str(space.n_tot) ] )
    log.disp ( ' ' )
    ...
    % Some logging header
    log.disp ( '***************************************************************' )
    log.disp ( 'Setting up one-dimensional grids for each degree of freedom    ' )
    log.disp ( 'Used for visualization only!                                   ' )
    log.disp ( '***************************************************************' )
    log.disp ( ' ' )
...
for k = 1:space.n_dim
    ...
    if isa (state,'traj')
        if ~isa(space.dof{k},'grid.fft')
            log.disp (['Found a non-FFT grid for dof : ' int2str(k)])
            log.error ('This is (currently!) not allowed for (quantum-)classical dynamics!')
        end
    end
