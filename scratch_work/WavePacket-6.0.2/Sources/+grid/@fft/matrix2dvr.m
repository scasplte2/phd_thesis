...
if length(size(fbr)) ~= 2*space.n_dim
    log.error('Input is not a proper matrix.')
end
...
expGrid_2ND = repmat(exp(-1i * obj.x_min * space.fbr{obj.dof}(:)), 1, space.n_tot);
...
expGrid_2ND = repmat(exp(-1i * obj.x_min * space.fbr{obj.dof}(:)'), space.n_tot, 1);
expGrid_2ND = reshape( expGrid_2ND, size(fbr) );
factorRight = sqrt(obj.x_max-obj.x_min) / obj.n_pts * expGrid_2ND;

dvr = fftshift ( fft(dvr, [], obj.dof + space.n_dim), obj.dof+space.n_dim );
dvr = dvr .* factorRight;
