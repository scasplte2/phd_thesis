...
classdef hermite < handle
    ...
    methods (Access = public) % in separate files within *this* directory
        ...
        function disp (obj) % Diplay method, overloading default disp method
            ...
            log.disp ( 'Discretization scheme: Gauss-Hermite DVR' )
            log.disp ( '***************************************************************' )
            log.disp ( ' ' )
            log.disp ( 'Parameters of the grid' )
            log.disp ( ['Number of grid points : ' int2str(obj.n_pts)])
            log.disp ( ['scaling factor        : ' num2str(sqrt(obj.mass*obj.omega))])
            log.disp ( ['harmonic frequency    : ' num2str(obj.omega)])
            log.disp ( ['position of minimum   : ' num2str(obj.r_e)  ])
            log.disp ( ' ' )
        end
