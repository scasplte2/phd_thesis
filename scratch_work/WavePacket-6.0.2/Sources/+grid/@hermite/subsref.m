...
if index.type == '.'
	switch index.subs
        ...
        case 'n_pts'
            retval = obj.n_pts;
        case 'dvr_min'
            retval = min(obj.x_grid);
        case 'dvr_max'
            retval = max(obj.x_grid);
        case 'fbr_min'
            retval = 0;
        case 'fbr_max'
            retval = obj.n_pts - 1;
		...
		otherwise
			log.error( ['unknown field ' index.subs ' of grid.hermite requested']);
	end
else
	log.error('Tried unsupported access form for class grid.hermite');
end
