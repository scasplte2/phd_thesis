...
classdef inter < efi.generic & handle
    ...
    methods (Access = public)
       ...
       function init_efi ( obj )
            ...
            init_efi@efi.generic ( obj );
            
        end
        
        % Display object (overwrite Matlab's default disp function)                        
        function disp(obj)
            log.disp ('Interpolated envelope')
            log.disp ('***************************************************************')
            log.disp (' ')
                log.disp(['Tabulated data from file     : '         obj.file])
                log.disp(['Interpolation method         : '         obj.method])
                log.disp(['Conversion factor for times  : ' num2str(obj.t_conv)])
                log.disp(['Conversion factor for fields : ' num2str(obj.f_conv)])
