...
log.disp('*********************************************')
log.disp('Control of vibrational modes of HF via       ')
log.disp('chirped infrared laser pulses.               ')
log.disp('                                             ')
log.disp('Figure 1 in the reference                    ')
log.disp('                                             ')
log.disp('see S. Chelkowski, A.D. Bandrauk, P.B. Corkum')
log.disp('Phys. Rev. Lett. 65, page 2355               ')
log.disp('*********************************************')
...
space.dof{1}.mass  = m_r;                % reduced mass of 1H-19F
...
time.steps.m_stop   = 460;               % index of last time step
...
% reference. Note that the reference makes an error of using S0/S wherever S/S0
% stands in the formulas
% A  = -0.5224;
B  = 0.0419;
Q  = 27734;
t0 = 16707;
t1 = 27217;
t12 = t1 + 2 * Q * (sqrt(13) - sqrt(2)); % eq. 12 in the reference
w0 = 2 * B * hamilt.pot{1,1}.d_e;        % harmonic frequency of the Morse potential
wi = w0 * (1 - B);                       % initial frequency of the pulse
wf = w0 - w0 * B * ( (t12-t1)^2 / (4*Q^2) + sqrt(2) * (t12-t1) / Q + 3/2);
                                         % final frequency of the pulse

T = (t0 + t12) /2;                       % delay time of the chirped pulse
...
time.pulse{1}.ampli     = sqrt(1/atomic.I.TW_cm2);
time.pulse{2}.ampli     = sqrt(1/atomic.I.TW_cm2);
time.pulse{3}.ampli     = sqrt(1/atomic.I.TW_cm2);
time.pulse{4}.ampli     = sqrt(1/atomic.I.TW_cm2);

time.pulse{1}.delay     = t0;            % delay time
time.pulse{2}.delay     = T;             % delay time
time.pulse{3}.delay     = t12+t0/10;     % delay time
time.pulse{4}.delay     = t12+t0/5;      % delay time
...
time.pulse{2}.fwhm      = t12-t0;         % FWHM (/length) of pulses
