...
for mval = 0:5
	...
	qm_init(state,mval);
	qm_propa(state);
	average{mval+1} = expect.amo{1}.cha{1};
end
...
figure(1);
clf;
plot(time.steps.m_grid, averagetot, 'k-', ...
     time.steps.m_grid, average{1}, 'k--', ...
     time.steps.m_grid, average{3}, 'k-.', ...
     time.steps.m_grid, average{5}, 'k:', ...
     'LineWidth', 2);
xlabel('t');
ylabel('<cos^2 \theta>');
