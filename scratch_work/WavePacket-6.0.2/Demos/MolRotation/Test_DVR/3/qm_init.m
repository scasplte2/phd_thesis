...
log.disp( '***********************************************' )
log.disp( 'Model system for testing spherical coordinate  ' )
log.disp( 'calculations. See model III of HCl in          ' )
log.disp( ' J.Chem.Phys 95, 7392 (1991)                   ' )
log.disp( '***********************************************' )
...
time.steps.m_stop   = 83;                % Index of final time step
...
hamilt.pot{1,1}     = pot.metiu2;        % model potential
hamilt.pot{1,1}.d_e = 37244/atomic.w.cm_1;    % dissociation energy
hamilt.pot{1,1}.r_e = 1.2746/atomic.l.A;      % equilibrium length
hamilt.pot{1,1}.alf = 2.38/(1.2746/atomic.l.A);% range parameter

% For qm_bound only
hamilt.eigen.stop = 150;
