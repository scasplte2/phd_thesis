...
global control hamilt plots space time reduce

log.disp ( '**********************************' )
log.disp ( 'Asymmetric double well potential  ' )
log.disp ( ' see example 2 in:                ' )
...
time.steps.m_delta = 1;                  % Size of time steps 
...
time.pulse{1}.fwhm  = 50;                % Full width at half maximum
time.pulse{1}.ampli = 3.75;              % u_0=3.75 [D/(e*a_0)] see section IV.C
time.pulse{1}.frequ = 0.196;             % Omega = 0.196 [D/hbar], see section IV.C
time.pulse{1}.phase = 9.8;               % Phase = frequ * delay = 0.196* 50 
...
% Ssave (bound) eigen states (==> qm_bound)
...
control.relax.rate = 0.373779924;        % Gamma_{2->0} should be = 1 [D/hbar]
                                         % set to 0.3738 due to error in ancestor code 
...
reduce.balance.A_shift = 1e-6;           % Shift spectrum of matrix A
reduce.balance.A_split = 1;              % dimension of instable part of A
reduce.balance.BN_scale = 8;             % Scaling factor for control fields
...
reduce.H2model.BN_scale = 4;             % Scaling factor for control fields
...
plots.expect.p_min = -0.05;              % Range of population plot
plots.expect.p_max =  0.8;               % Range of population plot
