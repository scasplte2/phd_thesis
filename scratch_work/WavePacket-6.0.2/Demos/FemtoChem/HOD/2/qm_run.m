...
% This is a demo script for doing optimal control theory with WavePacwave. We use
...
%
%    This setting is copied to time.init whenever we propagate our initial state
%    with one of the trial electric fields.
options.init.dof{1}.handle = init.gauss;
options.init.dof{1}.width  = 1/sqrt(4*23.18);
options.init.dof{1}.pos_0  = 1.806;
options.init.dof{1}.mom_0  = 0;

options.init.dof{2}.handle = init.gauss;
...
% Note the negative time, because we always use units of time.steps.m_delta (which
% is negative), i.e., time.steps.m_start * time.steps.m_delta should be the final
...
for iter = 1:options.iterations
    ...
    params.time.corr.handle = init.load;
    params.time.corr.dir    = options.save.dir;
    params.time.corr.file   = options.save.forward;
    params.hamilt.coupling.ini_rep = 'dia';
    params.psi.save.export      = true;
    params.psi.save.dir         = options.save.dir;
    ...
    %  the "initial state".
    context = wave.load(options.save.dir, options.save.forward, true);
    context = wave.load(context, 2);
    ...
    for tt = 1:context.time.steps.m_number-1
        wave.save(tt);
    end

    % the last wave function is the only one that is used by wav_load() anyway.
    psi.dvr.grid = feval(options.amo.handle, context.wf.grid);
    wave.save(context.time.steps.m_number);
    fclose('all');
    ...
    context_init = wave.load(options.save.dir, options.save.init);
    context_amo  = wave.load(options.save.dir, options.save.amo);

    efield = zeros(context_init.time.steps.m_number, 2);
    efield(:, 1) = context_init.time.steps.m_grid;

    for step = 1:size(efield, 1)
        context_init = wave.load(context_init, step);
        context_amo  = wave.load(context_amo, step);

        wf_init = context_init.wf.grid;
        wf_amo  = context_amo.wf.grid;
        trans_dipole = context_init.hamilt.d_x.grid{1,2};

        efield(step, 2) = 1i * (...
                sum( conj(wf_amo{2}(:)) .* trans_dipole(:) .* wf_init{1}(:) .* space.dvr.weight(:)) ...
                - sum( conj(wf_init{2}(:)) .* trans_dipole(:) .* wf_amo{1}(:) .* space.dvr.weight(:)) );
    end


    % E) Normalize the electric field and write it out to a file
    efield(:, 2) = efield(:, 2) * sqrt(options.efield.energy) ...
             / sqrt( sum(abs(efield(:,2)).^2) * abs(context_init.time.steps.m_delta) );
