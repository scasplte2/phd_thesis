...
qm_setup();
global atomic expect hamilt space
...
if length(dir('../1/state_*.dat')) ~= 2
    log.error(['The runscript expects the files state_g.dat and state_g.dat'...
...
for symmetry = ['g' 'u']
    ...
    state = wave;
	qm_init(state,symmetry);
    qm_bound(state);

    % an offset to store the ungerade states after the gerade ones.
    if symmetry == 'u'
        offset = hamilt.eigen.stop - hamilt.eigen.start + 1;
    else
        offset = 0;
    end
    ...
    % is constant 1 a.u.
    for step = hamilt.eigen.start+1:hamilt.eigen.stop+1
        eigen(state, step);

        positions(step - hamilt.eigen.start + offset) = expect.total(step) * atomic.w.cm_1;

        heights(step - hamilt.eigen.start + offset)  = ...
                abs( sum(conj(state.dvr{1}(:)) .* mystate .* space.weight(:)) ).^2;
    end
end
