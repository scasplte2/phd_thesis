...
for  m=1:hamilt.coupling.n_eqs
    ...
    if plots.density.energy.on
        contour ( space.dvr.grid_ND{1} , ...
                  ...
                  linspace( plots.pot.min, plots.pot.max, plots.density.contour.nlev(2) ), ...
                  'LineStyle', ':', ...
    ...
    if expect.ind.pop{m}(step)>expect.min_pop
        lower = plots.dvr.rho_max / plots.density.contour.nlev(1);
        upper = plots.dvr.rho_max;
        if ~isempty(plots.density.waves.lower)
            lower = plots.density.waves.lower;
        end
        if ~isempty(plots.density.waves.upper)
            upper = plots.density.waves.upper;
        end
...
if ~plots.density.marginals.on
    xlabel ( [ 'R_{', space.dof{1}.label, '} [a_0]' ] )
end
ylabel ( [ 'R_{', space.dof{2}.label, '} [a_0]' ] )

% Absorbing boundary conditions
if ~isempty(hamilt.nip.grid)
    if hamilt.nip.params.min(1) > space.dof{1}.dvr_min % Left
    ...
    if hamilt.nip.params.max(1) < space.dof{1}.dvr_max % Right
    ...
    if hamilt.nip.params.min(2) > space.dof{2}.dvr_min % Lower
        line ( [ space.dof{1}.dvr_min space.dof{1}.dvr_max ], ...
               [ hamilt.nip.params.min(2) hamilt.nip.params.min(2) ], ...
               'LineStyle', '--', ...
               'Color', 'k', ...
               'LineWidth', plots.style.line.thin)
    end
    
    if hamilt.nip.params.max(2) < space.dof{2}.dvr_max % Upper
        line ( [ space.dof{1}.dvr_min space.dof{1}.dvr_max ], ...
               [ hamilt.nip.params.max(2) hamilt.nip.params.max(2) ], ...
               'LineStyle', '--', ...
               'Color', 'k', ...
               'LineWidth', plots.style.line.thin)
    end
...
for  m=1:hamilt.coupling.n_eqs
    ...
    if plots.density.energy.on && strcmp(class(space.dof{1}), 'grid_fft') ...
       && strcmp(class(space.dof{2}), 'grid_fft')
        contour ( space.fbr.grid_ND{1} , ...
                  space.fbr.grid_ND{2}, ...
                  space.dof{1}.kin + space.dof{2}.kin, ...
                  linspace( plots.kin.min, plots.kin.max, plots.density.contour.nlev(2) ), ...
                  'LineStyle', ':', ...
                  'LineWidth', plots.style.line.thin, ...
                  'Color',     'k'  );
    end
    ...
    if expect.ind.pop{m}(step)>expect.min_pop
        ...
        contour ( space.fbr.grid_ND{1},    ...
                  space.fbr.grid_ND{2}, ...
                  abs(fbr).^2, ...
                  linspace(plots.fbr.rho_max/plots.density.contour.nlev(1), plots.fbr.rho_max, plots.density.contour.nlev(1)), ...   
                 'LineStyle', '-', ...
                 'LineWidth', plots.style.line.thin, ...
                 'Color',     plots.style.colors(m,:)   );
    end
...
title ( {info.header1;info.header2} )
if ~plots.density.marginals.on
    xlabel ( [ 'P_{', space.dof{1}.label, '} [hbar / a_0]' ] )
end
ylabel ( [ 'P_{', space.dof{2}.label, '} [hbar / a_0]' ] )
...
set ( gca, 'LineWidth',     plots.style.line.thick, ...
           ...
           'FontSize',      plots.style.font.large, ...
           'FontWeight',    plots.style.font.heavy )
xlabel ( [ 'P_{', space.dof{1}.label, '} [a_0]' ] )
...
ylabel ( [ 'P_{', space.dof{2}.label, '} [a_0]' ] )
if plots.density.range.on == false
