...
for m=1:hamilt.coupling.n_eqs
    ...
    if plots.density.energy.on
        ...
        if expect.ind.pop{m}(step)>expect.min_pop
            ...
            rho = min ( rho/plots.dvr.rho_max, 1 );
            too_small = find (rho<0.01);
            rho = rho + 2*m - 2;
...
if plots.density.energy.on
    zrange = [ plots.pot.min-plots.pot.delta/10 plots.pot.max+plots.pot.delta/10 ];
...
for m=1:hamilt.coupling.n_eqs
    ...
    % Surface plots of densities only (and color-coded phases)
        ...
        if expect.ind.pop{m}(step)>expect.min_pop
            ...
            % Shift individual densities vertically
            rho = abs   ( fbr ) .^2;
            rho = min ( rho/plots.fbr.rho_max, 1 );
            too_small = find (rho<0.01);
            rho = rho + 2*m - 2;
