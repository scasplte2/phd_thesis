...
subdiag = i * sqrt( (1:obj.n_pts-1) / 2 );

obj.momentum = zeros(obj.n_pts) + diag(subdiag, -1) + diag(conj(subdiag), 1);
obj.momentum = obj.momentum * sqrt(obj.omega * obj.mass);
...
obj.momentum = obj.trafo_reconstruct * obj.momentum * obj.trafo_expand;
...
obj.kin = obj.momentum * obj.momentum / (2*obj.mass);
...
obj.kinexpo = U * diag(exp(-i * time.sub.delta * fraction * diag(D))) * inv(U);
