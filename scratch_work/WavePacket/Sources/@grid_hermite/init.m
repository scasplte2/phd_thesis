...
obj.trafo_expand = zeros(obj.n_pts);
obj.trafo_reconstruct = zeros(obj.n_pts);
for n = 1:obj.n_pts
    obj.trafo_expand(n, :) = sqrt(factor) * weights .* exp(-(factor*(x_grid-obj.r_e)).^2/2)...
                             .* math.hermite(factor * (x_grid-obj.r_e), n-1);
    obj.trafo_reconstruct(:, n) = sqrt(factor) * exp(-(factor*(x_grid-obj.r_e)).^2/2) ...
                                  .* math.hermite(factor * (x_grid-obj.r_e), n-1);
end
