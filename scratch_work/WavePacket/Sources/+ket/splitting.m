...
%
%   (      d )                                         (   d )
% H (R, -i --) =  V (R) - F(t) * ( D (R) + D (R) ) + T (-i --)
%   (      dR)                      p       t          (   dR)
%
%
% with kinetic (T) and potential (V) energy and where the interaction  
% between the external electric field F(t) and the dipole moment D of
% the system is treated semiclassically. Here, the dipole matrix D 
...
% as well D_p(R) and D_t(R) can have two cartesian components (along 
% x,y) corresponding to different polarization directions.
%
...
% ==========
%
% Original version: J. A. Fleck et al., Appl. Phys., 10, 129 (1976)
%                   M. D. Feit, J. A. Fleck, A. Steiger, J. Comp. Phys. 47, 412 (1982) 
% Coupled equations: Alvarellos and Metiu
%
%-------------------------------------------------------------------------
...
if step==1
    ...
    if isfield(hamilt, 'kin')
        for n = 1:length(hamilt.kin)-1
            hamilt.kin{n} = init(hamilt.kin{n}, fraction, false);
        end
        hamilt.kin{end} = init(hamilt.kin{end}, 1, false);
    ...
    % Propagator for (permanent and/or transition) dipole energies
...
% After first step: Perform propagations
    ...
    for k = 1:time.sub.n
        ...
        % Permanent/transition dipoles: Full (Trotter) or half (Strang) sub-step
        if time.efield.n_pulse > 0
        ...
        if time.propa.params.order==3 
            ...
            % Transition/permanent dipoles: Half sub-step
            if time.efield.n_pulse>0
            ...
            % Potential energy: Half sub-step
            pot_propa;
