...
global expect hamilt plots psi space uncert time;
...
if isa(arg1, 'char') && isa(arg2, 'char')
	...
	context.cache = [];
	context.wf    = [];
    % if the additional parameter glob is true, set global variables
    ...
    for ii = 1:numel(times)
        ...
        if isempty(context.cache) || ind_file ~= context.cache.file_index
            filename = fullfile(context.psi.load.dir, ...
                    strcat(context.psi.load.file, '_', num2str(ind_file), '.mat'));
