...
if space.size.n_dim == 1
	hamilt.dip.params.R_dof = 1;
end

if space.size.n_dim == 2
	if ~isfield(hamilt.dip.params, 'R_dof') || isempty(hamilt.dip.params.R_dof)
		util.error('NaI dipole moment requires degree of freedom to be set.')
	end

	L_dof = 1;
	if hamilt.dip.params.R_dof == 1
		L_dof = 2
	end
	angleval = space.dvr.grid_ND{L_dof};
end

Rval = space.dvr.grid_ND{hamilt.dip.params.R_dof};
