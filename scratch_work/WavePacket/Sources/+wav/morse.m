...
% Output
util.disp ('  ')
util.disp (['Dissociation energy     : ' num2str(psi.init.dof{dir}.d_e)])
util.disp (['Equilibrium position    : ' num2str(psi.init.dof{dir}.r_e)])
util.disp (['Range parameter (alfa)  : ' num2str(psi.init.dof{dir}.alf)])
...
if (psi.init.dof{dir}.n_q>n_b)
    util.error ('quantum number exceeds number of bound states')
end

% Bound state energy
e_n = omega*(psi.init.dof{dir}.n_q+0.5) ...
    - chi*(psi.init.dof{dir}.n_q+0.5)^2 ...
    - psi.init.dof{dir}.d_e;
util.disp ('   ')
...
y = psi.init.dof{dir}.alf * ( space.dvr.grid_ND{dir}-psi.init.dof{dir}.r_e ); % Eq. 33
xi = 2*lambda*exp(-y); % Eq. 39
s = 2*lambda - 2*psi.init.dof{dir}.n_q - 1; % Eq. 46

% Recursion 
psi.n = ones(size(space.dvr.grid_ND{dir}));
if (psi.init.dof{dir}.n_q>0)
    psi.n1=zeros(size(space.dvr.grid_ND{dir}));
    for n=1:psi.init.dof{dir}.n_q
        psi.n2=psi.n1;
        psi.n1=psi.n;
...
scalog = ( log(xi)*s - xi + log(psi.init.dof{dir}.alf*s) - gammaln(s+1) ) / 2 ;
