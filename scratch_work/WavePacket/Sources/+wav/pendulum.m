...
switch psi.init.dof{dir}.multiple
    case 1 % Only even integer orders r
        if psi.init.dof{dir}.order~=2*round(psi.init.dof{dir}.order/2)
            util.error ('Order of Mathieu functions not compatible with m!')
        end
    case 2 % Only integer orders r 
        if psi.init.dof{dir}.order~=round(psi.init.dof{dir}.order)
            util.error ('Order of Mathieu functions not compatible with m!')
        end            
    otherwise
...
r = psi.init.dof{dir}.order;
q = psi.init.dof{dir}.barrier*2/psi.init.dof{dir}.multiple^2;
e = psi.init.dof{dir}.multiple*(space.dvr.grid_ND{dir}-psi.init.dof{dir}.shift)/2;

switch lower(psi.init.dof{dir}.parity)
    case ('c')
...
util.disp ( [ 'Mathieu order r      : ' int2str( psi.init.dof{dir}.order) ] )
util.disp ( [ 'Multiplicity m       : ' int2str( psi.init.dof{dir}.multiple) ] )
util.disp ( [ 'Potential barrier V0 : ' num2str( psi.init.dof{dir}.barrier) ] )
util.disp ( [ 'Potential shift R0   : ' num2str( psi.init.dof{dir}.shift) ] )
