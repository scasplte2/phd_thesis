...
if isfield(psi.init.dof{dir}, 'state')
    util.disp ( ['We pick the ' int2str(psi.init.dof{dir}.state) '-th eigenstate'])

    init_grid = zeros(size(space.fbr.grid_ND{dir}));
    init_grid( space.fbr.grid_ND{dir} == space.fbr.grid_1D{dir}(psi.init.dof{dir}.state)) = 1;

elseif isfield(psi.init.dof{dir}, 'coeffs')
    util.disp('Using user-specified coefficients')

    init_grid = zeros(size(space.fbr.grid_ND{dir}));
    for i = 1:length(psi.init.dof{dir}.coeffs)
        init_grid(space.fbr.grid_ND{dir} == space.fbr.grid_1D{dir}(i)) = psi.init.dof{dir}.coeffs(i);
    end
