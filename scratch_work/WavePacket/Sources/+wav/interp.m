...
util.disp (' ')
util.disp ('******************************************************')
util.disp ('Interpolation of initial wave function from tabulated ')
util.disp ('data.                                                 ')
util.disp ('******************************************************')
util.disp (['Conversion factor for coordinates : ' num2str(psi.init.corr.conv)])
util.disp (['Interpolation method              : ' psi.init.corr.method])
if space.size.n_dim>1
    if length(psi.init.corr.n_pts)~=space.size.n_dim
        error ('Inconsistent number of dimensions of tabulated data')
    end
    util.disp (['Number of data points in each dimension  : ' int2str(psi.init.corr.n_pts)])
else
    psi.init.corr.n_pts = 1; % Dummy
end
util.disp ( ' ' )
