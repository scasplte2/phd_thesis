...
options.save.dir    = '/scratch/ulorenz/HOD';
options.save.forward= 'forward';
options.save.init   = 'initial';
options.save.prj   = 'projected';
...
for iter = 1:options.iterations
    ...
    params.psi.init.corr.handle = @wav.load;
    params.psi.init.corr.dir    = options.save.dir;
    params.psi.init.corr.file   = options.save.forward;
    ...
    psi.dvr.grid_ND = feval(options.prj.handle, context.wf.grid_ND);
    ...
    params.psi.init.corr.handle = @wav.load;
    params.psi.init.corr.dir    = options.save.dir;
    params.psi.init.corr.file   = 'tmp';
    params.psi.init.representation = 'dia';
    params.psi.save.export      = true;
    params.psi.save.dir         = options.save.dir;
    params.psi.save.file        = options.save.prj;
    ...
    context_prj  = ket.load(options.save.dir, options.save.prj);
    ...
    for step = 1:size(efield, 1)
        ...
        context_prj  = ket.load(context_prj, step);

        wf_init = context_init.wf.grid_ND;
        wf_prj  = context_prj.wf.grid_ND;
        trans_dipole = context_init.hamilt.d_x.grid_ND{1,2};

        efield(step, 2) = 1i * (...
                sum( conj(wf_prj{2}(:)) .* trans_dipole(:) .* wf_init{1}(:) .* space.dvr.weight_ND(:)) ...
                - sum( conj(wf_init{2}(:)) .* trans_dipole(:) .* wf_prj{1}(:) .* space.dvr.weight_ND(:)) );
    end
