...
time.main.delta  = 41.341;           % Size of time steps: 1 fs 
...
time.efield.delay   = 2067.07;       % Time delay of pulse center: 50 fs
time.efield.fwhm    = 2067.07;       % Full width at half maximum: 50 fs
time.efield.ampli   = 0.0277002;     % Field amplitude: 142.44 MV/cm
time.efield.frequ   = 0.1607054;     % Carrier frequency: 35270.76 cm-1
...
hamilt.pot.params.pos_conv = 1.0;    % conversion factor for coordinates
hamilt.pot.params.pot_conv = 1.0;    % conversion factor for energies
hamilt.pot.params.method  = 'spline';% Cubic spline

hamilt.dip.handle      = @dip.interp;% Dipoles: Interpolate table
hamilt.dip.params.pos_conv = 1.0;    % conversion factor for coordinates
hamilt.dip.params.dip_conv = 1.0;    % conversion factor for dipole moments
hamilt.dip.params.method  = 'spline';% Cubic spline
...
hamilt.nip.params.max = +4.5;        % End of inner grid region

% Initial wave function
psi.init.dof{1}.handle = @wav.gauss; % Gaussian-shaped wavepacket
psi.init.dof{1}.width  =  0.1331;    % Width 
psi.init.dof{1}.pos_0  =  1.8601;    % Center in position representation
...
plots.density.range.on = true;       % manually set the ranges
