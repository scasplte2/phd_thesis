...
if step==1
    ...
    switch time.propa.params.order
        case (0)
            ...
            for ii = round(10*sqrt(time.main.alpha)) : -1 : round(sqrt(time.main.alpha))
                if abs(besseli(ii,time.main.alpha))>time.propa.params.precision
                    break
                end
            end
    ...
    time.main.coeffs = besseli([0:time.propa.params.order],time.main.alpha);
    ...
    time.main.expon = exp( -(hamilt.delta/2+hamilt.pot.min) * time.main.delta );
  
    
    
my_gs = gs;

coeff = sum( conj(my_gs(:)) .* psi.dvr.grid_ND{1}(:) .* space.dvr.weight_ND(:) );
psi.dvr.grid_ND{1} = psi.dvr.grid_ND{1} - coeff * my_gs;
...
psi.dvr.grid_ND{1} = psi.dvr.grid_ND{1} - coeff * my_gs;
    ket.hamilt(0,0,1);
coeff = sum( conj(my_gs(:)) .* psi.dvr.new_ND{1}(:) .* space.dvr.weight_ND(:) );
psi.dvr.new_ND{1} = psi.dvr.new_ND{1} - coeff * my_gs;
