...
hamilt.pot.params.v{1,1} = [0 0; 1 1];   % Force constants

% Periodic boundary conditions
hamilt.nip.handle = @nip.power;          % Negativ imaginary potential
hamilt.nip.params.exp  = [2 2];          % Exponent
hamilt.nip.params.min  = [-12.0 -12.0];  % Lower bound
hamilt.nip.params.max  = [+12.0 +12.0];  % Upper bound
...
psi.init.dof{2}.width = sqrt(1/2);       % Width 
psi.init.dof{2}.pos_0 = -5.0;            % Center in position representation
psi.init.dof{2}.mom_0 = -5.0;            % Center in momentum representation
