...
space.dof{3} = grid_fft;                 % Using FFT grid 
...
hamilt.pot.params.v{1,1} = [0 0 0;1 1 1];% Force constants
hamilt.pot.params.s{1,1} = [10  20  30 ];% Minimum positions


% Periodic boundary conditions
hamilt.nip.handle = @nip.power;          % Negativ imaginary potential
hamilt.nip.params.exp  = [2  2  2 ];     % Exponent
hamilt.nip.params.min  = [02 12 22];     % Lower bound
hamilt.nip.params.max  = [18 28 38];     % Upper bound

% Initial wave function
psi.init.dof{1}.handle= @wav.gauss;      % Gaussian-shaped wavepacket
psi.init.dof{1}.width = 1.5*sqrt(1/2);   % Width: squeezed state
psi.init.dof{1}.pos_0 = 10.0;            % Center in position representation
psi.init.dof{1}.mom_0 =  0.0;            % Center in momentum representation

psi.init.dof{2}.handle= @wav.gauss;      % Gaussian-shaped wavepacket
psi.init.dof{2}.width = sqrt(1/2);       % Width: coherent state
psi.init.dof{2}.pos_0 =  22.5;           % Center in position representation
psi.init.dof{2}.mom_0 =  +0.0;           % Center in momentum representation

psi.init.dof{3}.handle= @wav.gauss;      % Gaussian-shaped wavepacket
psi.init.dof{3}.width = sqrt(1/2);       % Width: coherent state
psi.init.dof{3}.pos_0 =  30.0;           % Center in position representation
psi.init.dof{3}.mom_0 =  -2.5;           % Center in momentum representation
