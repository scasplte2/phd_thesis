...
space.dof{1}       = grid_fft;           % Use Fourier grid
...
hamilt.pot.params.v{1,1} = [0; 1];       % Force constant

% Initial wave function
psi.init.dof{1}.handle= @wav.gauss;      % Gaussian-shaped wavepacket
psi.init.dof{1}.width = sqrt(2);         % Width 
...
plots.expect.energies.min = -2;          % plotting ranges for energy values
plots.expect.energies.max = +8;          % plotting ranges for energy values
% plots.expect.errorbar.on = true;
