...
if step==1
    ...
    if ~isfield (time.propa.params,'order')
        time.propa.params.order = 0;
    end
    
    switch time.propa.params.order
        case (0)
           
            % Search backward
            for ii = round(2.0*time.main.alpha) : -1 : 0.5*round(time.main.alpha)
                if abs(besselj(ii,time.main.alpha))>time.propa.params.precision
                    break
                end
            end
            time.propa.params.order = ii;
            util.disp (['Automatic truncation if coefficients drop below : ' num2str(time.propa.params.precision)])
            util.disp (['Number of polynomials required : ' int2str(time.propa.params.order)])
        otherwise
            util.disp (['Truncating polynomial series after : ' int2str(time.propa.params.order)])
    end
    util.disp ( ' ' )
    ...
    time.main.coeffs = besselj([0:time.propa.params.order],time.main.alpha);
    time.main.coeffs(2:time.propa.params.order+1) = 2 * time.main.coeffs(2:time.propa.params.order+1);

    % Get coefficients (columns) also for time sub steps (rows)
    time.sub.alpha =  [1:time.sub.n]' * time.sub.delta * hamilt.delta/2;
   
    % Suddenly(?), BESSELJ  no longer accepts mixtures of row and column vectors
    [NU1,Z1] = meshgrid ([0:time.propa.params.order],time.sub.alpha);
    time.sub.coeffs  = besselj(NU1,Z1);
    % time.sub.coeffs  = besselj([0:time.propa.params.order],time.sub.alpha);
    time.sub.coeffs (:,2:time.propa.params.order+1) = 2 * time.sub.coeffs(:,2:time.propa.params.order+1);

    % Phase factors to compensate for normalization of Hamiltonian
    time.main.phase = exp( -1i * (hamilt.delta/2+hamilt.pot.min) *                   time.main.delta );
    time.sub.phase  = exp( -1i * (hamilt.delta/2+hamilt.pot.min) * [1:time.sub.n]' * time.sub.delta );
    ...
    %-----------------------------------------------
    for k=2:time.propa.params.order
