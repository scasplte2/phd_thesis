...
if space.size.n_dim>1
    if length(hamilt.dip.params.n_pts)~=space.size.n_dim
        error ('Inconsistent number of dimensions of tabulated data')
    end
    util.disp (['Number of data points expected    : ' int2str(hamilt.dip.params.n_pts)])
else
    hamilt.dip.params.n_pts = 1; % Dummy
end
util.disp ( ' ' )
            ...
            if fid == -1
                ...
                if p=='x'
                    hamilt.d_x.grid_ND{m,n} = util.interp (...
                        space.dvr.grid_ND, ...
                        filename, ...
                        hamilt.dip.params.n_pts, ...
                        hamilt.dip.params.method, ...
                        hamilt.dip.params.pos_conv, ...
                        hamilt.dip.params.dip_conv);
                elseif p=='y'
