...
if step==1
    ...
    for k = 1:space.size.n_dim
        plots.kin.max = plots.kin.max + min(space.dof{k}.kin_max, hamilt.truncate.delta);
    end
    if isfield(hamilt, 'kin')
        for k = 1:length(hamilt.kin)
            plots.kin.max = plots.kin.max + min(hamilt.kin{k}.kin_max, hamilt.truncate.delta);
        end
    end
    plots.kin.delta = plots.kin.max - plots.kin.min;

    if isempty(plots.pot.min)
        plots.pot.min = hamilt.pot.min;
    end
    if isempty(plots.pot.max)
        plots.pot.max = hamilt.pot.max;
    end
    if plots.pot.min==plots.pot.max % Dirty trick for free particle
        plots.pot.max = plots.kin.max;
    end
    plots.pot.delta = plots.pot.max - plots.pot.min;

    plots.tef.min = min(plots.pot.min,0);
    plots.tef.max = plots.pot.max + plots.kin.max;
    plots.tef.delta = plots.tef.max - plots.tef.min;
    ...
    if plots.density.on && space.size.n_dim <= 2
        ...
        if isempty(plots.dvr.rho_max)
            plots.dvr.rho_max = dvr_max;
        end
        if isempty(plots.fbr.rho_max)
            plots.fbr.rho_max = fbr_max;
        end
...
if plots.density.on
    ...
    if step==time.main.n
        if plots.density.export.on
            ...
            util.disp ( ['Saving first snapshot of animation to file : ' full_file_name] )
            util.disp ( '*******************************************************************' )
            util.disp (' ')
