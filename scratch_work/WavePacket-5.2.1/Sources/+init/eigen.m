...
if isfield(hamilt.eigen, 'symmetry')
    ...
    switch class(space.dof{1})
        ...
        case {'grid_legendre','grid_hermite'}
            main = eye ( N/2);
            anti = fliplr(main);
        ...
        case 'grid_fft' % Note absence of grid point at space.dof{1}.r_max
            switch lower(hamilt.eigen.symmetry)
                ...
                case '1'
                    util.disp ('1st (A_g) irrep of C_2h group - FFT')
                    main = eye ( N/4-1 );
                    anti = fliplr(main);
                ...
                case '2'
                    util.disp ('2nd irrep of C_2h group - FFT')
                    main = eye ( N/4-1 );
                    anti = fliplr(main);
                    init = zeros(1,N); init(1)=1; init(N/2+1)=-1;
                    fill = zeros(N/4-1,1);
                ...
                case '3'
                    util.disp ('3rd irrep of C_2h group - FFT')
                    main = eye ( N/4-1 );
                    anti = fliplr(main);
                    last = zeros(1,N); last(N/4+1)=1;last(3*N/4+1)=-1;
                    fill = zeros(N/4-1,1);
                ...
                case '4'
                    util.disp ('4th irrep of C_2h group - FFT')
                    main = eye ( N/4-1 );
                    anti = fliplr(main);
                    fill = zeros(N/4-1,1);
                    hamilt.eigen.transform = [fill main fill -anti fill main fill -anti];
                otherwise
                    util.error ('Symmetry should be "g" (=even) or "u" (=odd) or 1,2,3,4 for FFT')
            end
