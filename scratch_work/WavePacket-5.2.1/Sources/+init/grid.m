...
% Tell each DVR which degree of freedom it represents.
for k = 1:space.size.n_dim
    space.dof{k}.dof = k;
end
...
% where the cell index denotes the DOF
% Note that this call also generates log file output.
for k = 1:space.size.n_dim
    [space.dof{k} space.dvr.weight_1D{k} space.dvr.grid_1D{k} space.fbr.grid_1D{k}] = init(space.dof{k});
end
