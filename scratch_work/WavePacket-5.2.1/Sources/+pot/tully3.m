...
hamilt.pot.grid_ND{1,1} = + hamilt.pot.params.A * ones(size(space.dvr.grid_ND{1}));
hamilt.pot.grid_ND{2,2} = - hamilt.pot.params.A * ones(size(space.dvr.grid_ND{1}));

left  = find ( space.dvr.grid_ND{1}< 0 );
right = find ( space.dvr.grid_ND{1}>=0 );
hamilt.pot.grid_ND{1,2}(left)  = + hamilt.pot.params.B *       exp (   hamilt.pot.params.C * space.dvr.grid_ND{1}(left)  );
hamilt.pot.grid_ND{1,2}(right) = + hamilt.pot.params.B * ( 2 - exp ( - hamilt.pot.params.C * space.dvr.grid_ND{1}(right) ) );
