...
util.disp( ['Bending angle       : ' num2str(hamilt.pot.params.theta)] )
for ii = 1:3
    util.disp( '' )
    util.disp( ['Bond length #       : ' num2str(ii)                       ] )
    util.disp( ['dissociation energy : ' num2str(hamilt.pot.params.d_e(ii))] )
    util.disp( ['equilibrium distance: ' num2str(hamilt.pot.params.r_e(ii))] )
    util.disp( ['range parameter     : ' num2str(hamilt.pot.params.a(ii))  ] )
    util.disp( ['Sato parameter      : ' num2str(hamilt.pot.params.s(ii))  ] )
end
...
% shortcut
param = hamilt.pot.params;

% Atomic distances
R{1} = space.dvr.grid_ND{1};
R{2} = space.dvr.grid_ND{2};
R{3} = sqrt(R{1}.^2 + R{2}.^2 - 2*R{1}.*R{2}*cos(hamilt.pot.params.theta));
