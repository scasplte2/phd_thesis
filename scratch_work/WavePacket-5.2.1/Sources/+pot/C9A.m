...
if strcmp(class(space.dof{1}), 'grid_legendre')
...
if ~isfield(hamilt.pot, 'params') || ~isfield(hamilt.pot.params, 'state')
    switch hamilt.coupling.n_eqs
        case 1
            hamilt.pot.params.state = 1;
        case 2
            hamilt.pot.params.state = [2 3];
        case 3
            hamilt.pot.params.state = [1 2 3];
        otherwise
            util.error('This potential is only for up to three surfaces')
    end
end

if length(hamilt.pot.params.state) ~= hamilt.coupling.n_eqs
...
util.disp ('***********************************************')
util.disp ( ['State(s)    : ' num2str(hamilt.pot.params.state)])
util.disp ('')
...
for m = 1:hamilt.coupling.n_eqs
    state = hamilt.pot.params.state(m);
    if state == 2
