...
util.disp ( 'adiabatic representation               ' )
util.disp ( '***************************************' )
...
hamilt.pot.params.omega=  0.0;           % Harmonic frequency
hamilt.pot.params.kappa=  1.0;           % Linear JT coupling
hamilt.pot.params.gamma=  0.0;           % Quadratic JT coupling

hamilt.nip.handle      = @nip.power;     % Negative imaginary potential
hamilt.nip.params.exp  = [4 4];          % Exponent
hamilt.nip.params.min  = [-08.0 -08.0];  % Lower bound
hamilt.nip.params.max  = [+08.0 +08.0];  % Upper bound

% Initial wave function
psi.init.dof{1}.handle= @wav.gauss;      % Gaussian-shaped wavepacket
psi.init.dof{1}.width =  0.7;            % Width 
psi.init.dof{1}.pos_0 = -4.0;            % Center in position representation
psi.init.dof{1}.mom_0 =  4.0;            % Center in momentum representation

psi.init.dof{2}.handle= @wav.gauss;      % Gaussian-shaped wavepacket
psi.init.dof{2}.width =  1.0;            % Width 
...
plots.density.surface.view  = [10 27];   % View point for surface plot: [az el]
