...
util.disp ( 'Dynamics on two coupled states            ' )
util.disp ( 'Including geometric (Berry) phase effect !' )
util.disp ( '******************************************' )
...
hamilt.pot.params.omega=  5.0;       % Harmonic frequency
hamilt.pot.params.kappa= 10.0;       % Linear JT coupling
hamilt.pot.params.gamma=  0.0;       % Quadratic JT coupling

% Initial wave function
psi.init.dof{1}.handle= @wav.gauss;  % Gaussian-shaped wavepacket
psi.init.dof{1}.width =  0.5;        % Width 
psi.init.dof{1}.pos_0 = -2.0;        % Center in position representation
psi.init.dof{1}.mom_0 =  0.0;        % Center in momentum representation

psi.init.dof{2}.handle= @wav.gauss;  % Gaussian-shaped wavepacket
psi.init.dof{2}.width =  0.5;        % Width 
psi.init.dof{2}.pos_0 =  0.0;        % Center in position representation
psi.init.dof{2}.mom_0 =  0.0;        % Center in momentum representation

psi.init.representation = 'dia';     % initial WF in diabatic representation
psi.init.coeffs       = [1 0];       % Initially lower diabatic state populated


% Modify settings for appearance of plots (if desired)
plots.density.type = 'surface';

plots.density.surface.view  = [90 -90];  % Bottom view (without potentials)
...
plots.expect.energies.min   = -10;       % manual setting of minimum of energy expct. plot
plots.expect.energies.max   = 7;         % manual setting of maximum of energy expct. plot

plots.expect.population.min = -0.05;
plots.expect.population.max = 0.05;
