...
util.disp ( '***************************************************' )
util.disp ( 'Single crossing example, k=8.2 (Tully 1990)' )
util.disp ( '***************************************************' )
...
time.main.stop  = 030;               % Index of final time step
...
hamilt.pot.params.A    = 0.01;       % Tully's parameter A
hamilt.pot.params.B    = 1.6;        % Tully's parameter B
hamilt.pot.params.C    = 0.005;      % Tully's parameter C
hamilt.pot.params.D    = 1.0;        % Tully's parameter D

% Absorbing boundary conditions
hamilt.nip.handle      = @nip.power; % Negative imaginary potential
hamilt.nip.params.exp  = 3;          % Exponent
hamilt.nip.params.min = -8;          % Beginning of inner grid region
hamilt.nip.params.max =  8;          % End of inner grid region

% Initial wave function
psi.init.dof{1}.handle= @wav.gauss;  % Gaussian-shaped wavepacket
psi.init.dof{1}.width =  0.75;       % Width 
psi.init.dof{1}.pos_0 = -6.0;        % Center in position representation
psi.init.dof{1}.mom_0 =  8.2;        % Center in momentum representation
...
plots.expect.energies.max = 0.02;
