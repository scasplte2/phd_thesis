...
util.disp( '\sigma = 0.5 pulse and a single molecule       ' )
...
space.dof{1}.l_max = 20;                 % maximum angular momentum
...
time.main.delta = 0.01;                  % Size of main time steps
time.sub.n      = 50;                    % Number of sub steps per main step
...
time.efield.delay   = 1.5 ;              % pulse delay, measured from paper
time.efield.fwhm    = 0.5*sqrt(4*log(2));% FWHM of pulse; in fact it is the intensity we
                                         % transparently use here... sigma is 0.5
...
hamilt.dip.handle = @dip.taylor;         % Taylor series in cos theta
