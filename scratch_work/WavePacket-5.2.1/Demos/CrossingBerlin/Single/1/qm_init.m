...
hamilt.pot.params.repuls  = 1.0;     % Repulsion (exponential)
hamilt.pot.params.attrac  = 1.0;     % Attraction (parabolic)
hamilt.pot.params.couple  = 0.1;     % Coupling (constant)

% Absorbing boundary conditions
hamilt.nip.handle      = @nip.power; % Negative imaginary potential
hamilt.nip.params.exp  = 4;          % Exponent
hamilt.nip.params.min = +0.15;       % Beginning of inner grid region
hamilt.nip.params.max = +2.50;       % End of inner grid region

% Initial wave function
psi.init.dof{1}.handle= @wav.gauss;  % Gaussian-shaped wavepacket
psi.init.dof{1}.width =  0.05;       % Width 
psi.init.dof{1}.pos_0 =  0.4;        % Center in position representation
psi.init.dof{1}.mom_0 =  100;        % Center in momentum representation
