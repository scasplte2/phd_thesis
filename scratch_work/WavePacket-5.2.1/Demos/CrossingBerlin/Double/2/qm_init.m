...
hamilt.pot.params.c = cell(2);           % Pre-allocate cell array
hamilt.pot.params.v = cell(2);           % Pre-allocate cell array
hamilt.pot.params.c{1,1} = 0.1;          % Constant potential
hamilt.pot.params.v{2,2} = [0;2*0.05];   % Parabolic potential
hamilt.pot.params.c{1,2} = 0.02;         % Coupling (constant)

% Absorbing boundary conditions
hamilt.nip.handle      = @nip.power;     % Negative imaginary potential
hamilt.nip.params.exp  = 4;              % Exponent
hamilt.nip.params.min = -4.0;            % Beginning of inner grid region
hamilt.nip.params.max = +4.0;            % End of inner grid region

% Initial wave function
psi.init.dof{1}.handle= @wav.gauss;  % Gaussian-shaped wavepacket
psi.init.dof{1}.width =  0.2991/2;   % Width 
psi.init.dof{1}.pos_0 = -3.0;        % Center in position representation
psi.init.dof{1}.mom_0 = 20.0;        % Center in momentum representation
